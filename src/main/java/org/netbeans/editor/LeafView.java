...
public class LeafView extends BaseView {
	...
	protected void paintAreas(Graphics g, int clipY, int clipHeight, int paintAreas) {
		if ((paintAreas & MAIN_AREA) == MAIN_AREA) {
			...
			int paintY = Math.max(clipY, 0); // relative start of area to
												// paint
			...
			if (clipHeight > 0) { // needs to be painted
				...
				try {
					...
					DrawEngine.getDrawEngine().draw(new DrawGraphics.GraphicsDG(g), editorUI, startPos, endPos, getBaseX(baseY), baseY, Integer.MAX_VALUE);
	...
	protected int getYFromPos(int pos) throws BadLocationException {
		int relLine = 0;
		try {
			relLine = ((BaseDocument) getDocument()).op.getLine(pos) - ((BaseElement) getElement()).getStartMark().getLine();
		} catch (InvalidMarkException e) {
			if (Boolean.getBoolean("netbeans.debug.exceptions")) { // NOI18N
				e.printStackTrace();
			}
