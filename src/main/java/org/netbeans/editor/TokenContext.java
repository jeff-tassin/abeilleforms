 ...
 * can be used. This set can be retrieved by calling the <tt>getTokenIDs()</tt>
 ...
 * syntax-class is known using the <tt>get</tt> method.
 * 
 * 
...
public class TokenContext {
	 ...
	 * 
	 * @param namePrefix
	 *            name that will prefix all the token-ids names.
	 * @param children
	 *            child token contexts.
	 */
	...
	/**
	 * Get the children contexts of this context. It returns empty-array if
	...
	/**
	 * Add all static-final token-id fields declared in this token-context using
	 * <tt>Class.getDeclaredFields()</tt> call.
	 */
	protected void addDeclaredTokenIDs() throws IllegalAccessException, SecurityException {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if ((fields[i].getModifiers() & flags) == flags && TokenID.class.isAssignableFrom(fields[i].getType())) {
	...
	/**
	 * Get the token-ids that belong to this token-context. It doesn't return
	 * the children's token-ids.
	 */
	...
	public TokenCategory[] getTokenCategories() {
		if (tokenCategories == null) {
			synchronized (tokenCategoryList) {
				tokenCategories = (TokenCategory[]) tokenCategoryList.toArray(new TokenCategory[tokenCategoryList.size()]);
			}
		}
	...
	/**
	 * Get the context path for this token-context that is derived from the path
	...
	/**
	 * Get all the context paths for this token-context including itself as the
