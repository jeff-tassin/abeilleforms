...
public class SyntaxSupport {
	 ...
	 */
	public synchronized int[] getTokenBlocks(int startPos, int endPos, TokenID[] tokenIDArray) throws BadLocationException {
		doc.readLock();
		try {
			...
			try {
				doc.op.prepareSyntax(slot, syntax, doc.op.getLeftSyntaxMark(startPos), startPos, endPos - startPos, true);
	...
	public void initSyntax(Syntax syntax, int startPos, int endPos, boolean forceLastBuffer, boolean forceNotLastBuffer) throws BadLocationException {
		...
		try {
			...
			doc.op.prepareSyntax(slot, syntax, doc.op.getLeftSyntaxMark(startPos), startPos, 0, forceLastBuffer, forceNotLastBuffer);
			int preScan = syntax.getPreScan();
	...
	public void tokenizeText(TokenProcessor tp, int startOffset, int endOffset, boolean forceLastBuffer) throws BadLocationException {
		...
		try {
			...
			int docLen = doc.getLength();
			doc.op.prepareSyntax(slot, syntax, doc.op.getLeftSyntaxMark(startOffset), startOffset, endOffset - startOffset, forceLastBuffer);
			int preScan = syntax.getPreScan();
			tp.nextBuffer(slot.array, syntax.getOffset(), endOffset - startOffset, startOffset, preScan, syntax.lastBuffer);
