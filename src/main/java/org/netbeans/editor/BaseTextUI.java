...
public class BaseTextUI extends TextUI implements ViewFactory, PropertyChangeListener, DocumentListener {
	...
	protected void modelChanged(BaseDocument oldDoc, BaseDocument newDoc) {
		...
		if (newDoc != null) {
			...
			// Execute actions related to document installaction into the
			// component
			Settings.KitAndValue[] kv = Settings.getValueHierarchy(kit.getClass(), SettingsNames.DOC_INSTALL_ACTION_NAME_LIST);
			for (int i = kv.length - 1; i >= 0; i--) {
				List actList = (List) kv[i].value;
				actList = kit.translateActionNameList(actList); // translate
																// names to
																// actions
				if (actList != null) {
	...
	public void installUI(JComponent c) {
		if (c instanceof JTextComponent) {
			...
			component.setCaretColor(Color.black); // will be changed by
													// settings later
			component.setCaret(caret);

			// assign blink rate
			int br = SettingsUtil.getInteger(Utilities.getKitClass(component), SettingsNames.CARET_BLINK_RATE, SettingsDefaults.defaultCaretBlinkRate
					.intValue());
	...
	public Dimension getPreferredSize(JComponent c) {
		...
		BaseDocument doc = (c instanceof JTextComponent) ? Utilities.getDocument((JTextComponent) c) : null;
		if (doc != null) {
			doc.readLock();
		}
		try {
			...
			d.width = (int) Math.min(rootView.getPreferredSpan(View.X_AXIS) + i.left + i.right + margin.left + margin.right, MAX_SPAN);
			d.height = (int) Math.min(rootView.getPreferredSpan(View.Y_AXIS) + i.top + i.bottom + margin.top + margin.bottom, MAX_SPAN);
		} finally {
			if (doc != null) {
				doc.readUnlock();
			}
	...
	public Dimension getMinimumSize(JComponent c) {
		...
		try {
			d.width = (int) rootView.getMinimumSpan(View.X_AXIS) + i.left + i.right + margin.left + margin.right;
			d.height = (int) rootView.getMinimumSpan(View.Y_AXIS) + i.top + i.bottom + margin.top + margin.bottom;
		} finally {
			if (doc != null) {
				doc.readUnlock();
			}
	...
	public Dimension getMaximumSize(JComponent c) {
		...
		try {
			d.width = (int) Math.min(rootView.getMaximumSpan(View.X_AXIS) + i.left + i.right + margin.left + margin.right, MAX_SPAN);
			d.height = (int) Math.min(rootView.getMaximumSpan(View.Y_AXIS) + i.top + i.bottom + margin.top + margin.bottom, MAX_SPAN);
		} finally {
			if (doc != null) {
				doc.readUnlock();
			}
	...
	/**
	 * Next visually represented model location where caret can be placed. This
	 * version works without placing read lock on the document.
	 */
	...
	/**
	 * This method gets called when a bound property is changed. We are looking
	...
	public void insertUpdate(DocumentEvent evt) {
		...
		if (((BaseDocumentEvent) evt).getLFCount() > 0 || evt.getLength() == 0) {
	...
	public void changedUpdate(DocumentEvent evt) {
		if (evt instanceof BaseDocumentEvent) {
			...
			if (layerName != null) {
				getEditorUI().addLayer(doc.findLayer(layerName), bdevt.getDrawLayerVisibility());
			}
	...
	class RootView extends BaseView {
		...
		/**
		 * Sets the only one inner view of root view to either null or a valid
		 * view.
		 */
		void setView(View v) {
			if (v instanceof BaseView) { // only BaseView instances are
											// supported
				if (view != null) {
					// enable grb.col.
					view.setParent(null);
		...
		/**
		 * Fetches the allocation for the given child view. Returns the whole
		 * allocated area.
		 */
		...
		/**
		 * Determine next visually represented model location where caret can be
		 * placed.
		 */
