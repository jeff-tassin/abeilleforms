...
/**
 * Block of text created using two marks. These blocks can be chained. Existing
...
public class MarkBlock {
	...
	/**
	 * This value is not used directly in this class but can be used by other
	...
	/**
	 * Single bit value that means that tested block and THIS block partially or
	...
	/**
	 * Single bit value that means that the tested block doesn't overlap with
	...
	/**
	 * Tested block completely before THIS mark block but its end position
	...
	/**
	 * Tested block completely after THIS mark block but its start position
	...
	/**
	 * Tested block partly covers begining of THIS mark block. The value
	 * includes OVERLAP.
	 */
	public static final int OVERLAP_BEGIN = 256 | OVERLAP;

	/**
	 * Tested block partly covers end of THIS mark block. The value includes
	 * OVERLAP.
	 */
	public static final int OVERLAP_END = 512 | OVERLAP;

	/**
	 * Start position of the tested block is lower than the start position of
	...
	/**
	 * End position of the tested block is greater than the end position of THIS
	...
	/**
	 * Tested block fully includes THIS block and extends it by at least one
	 * character in both directions. The value is EXTEND_BEGIN | EXTEND_END.
	 */
	public static final int INCLUDE = EXTEND_BEGIN | EXTEND_END;

	/**
	 * Tested block completely inside THIS block and its end position is lower
	...
	/**
	 * Tested block completely inside THIS block and its start position is
	...
	/**
	 * Tested block is fully inside THIS block and there is at least one more
	...
	/**
	 * The blocks have exactly the same start and end positions. They simply
	 * cover exactly the same area. The value is INSIDE_BEGIN | INSIDE_END.
	 */
	public static final int SAME = INSIDE_BEGIN | INSIDE_END;

	/**
	 * This value can be used to clear the two bits saying if the tested or THIS
	 ...
	 * 
	 * @param startPos
	 *            starting position of the compared block
	 * @param endPos
	 *            ending position of the compared block or it is the same as
	 *            startPos when testing just for insert
		...
		try {
			...
			if (startPos == endPos) { // tested empty
				if (startThis == endThis) { // both empty
					...
					if (startPos <= startThis) {
						return (startPos < startThis) ? (EMPTY | BEFORE) : (EMPTY | INSIDE_BEGIN);
					}
					else if (startPos >= endThis) {
						return (startPos > endThis) ? (EMPTY | AFTER) : (EMPTY | INSIDE_END);
					}
					else {
						return EMPTY | INNER;
					}
			...
			if (startThis == endThis) { // this empty, tested non-empty
				if (startPos >= startThis) {
					return (startPos > startThis) ? (THIS_EMPTY | AFTER) : (THIS_EMPTY | EXTEND_END);
				}
				else if (endPos >= startThis) {
					return (endPos > startThis) ? (THIS_EMPTY | BEFORE) : (THIS_EMPTY | EXTEND_BEGIN);
				}
				else {
	...
	/**
	 * Extend this mark block by start and end positions. First test whether the
	...
	public boolean extend(int startPos, int endPos, boolean concat) throws BadLocationException {
		try {
			...
			if ((rel & OVERLAP_BEGIN) == OVERLAP_BEGIN || ((rel & CONTINUE_BEGIN) == CONTINUE_BEGIN && concat)) {
				extended = true;
				doc.op.moveMark(startMark, startPos);
			}
			if ((rel & OVERLAP_END) == OVERLAP_END || ((rel & CONTINUE_END) == CONTINUE_END && concat)) {
				extended = true;
				doc.op.moveMark(endMark, endPos);
			}
			return extended;
	 ...
	 * 
	 * @return whether the block was extended. If it was, the caller is
	 *         responsible for possibly removing blk from the chain
	 */
	...
	/**
	 * Shrink this mark block by the block specified. startMark is moved to the
