...
public class MultiKeyBinding extends JTextComponent.KeyBinding implements java.io.Externalizable {

	/**
	 * Successive keystroke. They must be pressed in the order they are stored
	 ...
	 * 
	 * @param keys
	 *            successive keystroke that must be pressed in order to invoke
	 *            action
	 * @param actionName
	 *            action that will be invoked. Action is resolved from name by
	 *            calling kit.getActions() after the kit is constructed
	 */
	...
	public boolean equals(Object o) {
		if (o instanceof MultiKeyBinding) {
			...
			if (keys == null) {
				if (kb.keys == null) {
					return (key == null && kb.key == null) || (key != null && key.equals(kb.key));
				}
				else {
					return (kb.keys.length == 1 && ((key == null && kb.keys[0] == null) || (key != null && key.equals(kb.keys[0]))));
				}
			}
			else { // keys != null
				if (kb.keys != null) {
					...
					return (keys.length == 1 && ((kb.key == null && keys[0] == null) || (kb.key != null && kb.key.equals(keys[0]))));
				}
			}
		}
		return false;
	...
	/**
	 * Add or replace key bindings array by changes given in the second bindings
	 * array
	 * 
	 * @param target
	 *            target list of bindings
	 * @param changes
	 *            list of changes to apply: binding containing the non-null
	 *            keystroke(s) and non-null action will add the binding or
	 *            replace the old binding with the same keystroke(s) in the
	 *            target array, binding of the non-null keystroke(s) and null
	 *            action removes the binding for that keystroke from the target
	 *            array (if it existed) binding containing null keystroke and
	 *            non-null action adds or replaces default action
	 */
