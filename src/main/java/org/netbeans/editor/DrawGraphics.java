...
interface DrawGraphics {
	...
	/**
	 * Inform the draw-graphics about the current background color of the
	 * component.
	 */
	...
	/**
	 * Get the AWT-graphics to determine whether this draws to a graphics. This
	 * is useful for fast line numbering and others.
	 */
	public Graphics getGraphics();

	/**
	 * Whether draw graphics supports displaying of line numbers. If not line
	...
	/**
	 * Called when whole drawing ends. Can be used to deallocate some resources
	 * etc.
	 */
	public void finish();
	 ...
	 * 
	 * @param width
	 *            width of the rectangle to fill in points. The current
	 ...
	 * 
	 * @param offset
	 *            offset in the buffer for drawn text; if the text contains
	 *            tabs, then offset is set to -1 and length contains the count
	 *            of the space characters that correspond to the expanded tabs
	 * @param length
	 *            length of the text being drawn
	 * @param width
	 *            width of the text being drawn in points. The current
	 *            x-coordinate must be increased by width automatically.
	 * @param strikeThroughColor
	 *            color to be used for strike-through or null if the
	 *            strike-through shouldn't be done.
	 * @param underlineColor
	 *            color to be used for underlining or null if the underlining
	 *            shouldn't be done.
	 */
	public void drawChars(int offset, int length, int width, Color strikeThroughColor, Color underlineColor);

	/**
	 * Draw the expanded tab characters.
	 * 
	 * @param offset
	 *            offset in the buffer where the tab characters start.
	 * @param length
	 *            number of the tab characters
	 * @param spaceCount
	 *            number of spaces that replace the tabs
	 * @param width
	 *            width of the spaces in points. The current x-coordinate must
	 *            be increased by width automatically.
	 * @param strikeThroughColor
	 *            color to be used for strike-through or null if the
	 *            strike-through shouldn't be done.
	 * @param underlineColor
	 ...
	 *            shouldn't be done.
	 */
	public void drawTabs(int offset, int length, int spaceCount, int width, Color strikeThroughColor, Color underlineColor);

	/** Set character buffer from which the characters are drawn. */
	public void setBuffer(char[] buffer);

	/**
	 * This method is called to notify this draw graphics in response from
	 * targetPos parameter passed to draw().
	 * 
	 * @param offset
	 *            position that was reached during the drawing.
	 * @param ch
	 *            character at offset
	 * @param charWidth
	 *            visual width of the character ch
	 * @param ctx
	 *            current draw context containing
	 * @return whether the drawing should continue or not. If it returns false
	 *         it's guaranteed that this method will not be called again and the
	 *         whole draw() method will be stopped. <BR>
	 *         The only exception is when the -1 is used as the target offset
	 *         when draw() is called which means that every offset is a
	 *         potential target offset and must be checked. In this case the
	 *         binary search is used when finding the target offset inside
	 *         painted fragment. That greatly improves performance for long
	 *         fragments because the font metrics measurements are relatively
	 *         expensive.
	 */
	...
	/**
	 * Abstract draw-graphics that maintains a fg and bg color, font, current x
	 * and y coordinates.
	 */
	...
	/**
	 * Implementation of DrawGraphics to delegate to some Graphics. It optimizes
	 * the drawing by joining together the pieces of the text drawn with the
	 * same font and fg/bg color.
	 */
	static final class GraphicsDG extends SimpleDG {
		...
		/**
		 * Start of the chars that were not drawn yet. It can be -1 to indicate
		 * the buffered characters were just flushed.
		 */
		...
		/**
		 * Access to annotations for this document which will be drawn on the
		 * background
		 */
		...
		private void flush() {
			...
			graphics.drawChars(buffer, startOffset, endOffset - startOffset, startX, startY + lineAscent);
			...
			if (strikeThroughColor != null) { // draw strike-through
				...
				graphics.fillRect(startX, startY + (int) (fmcInfo.getStrikethroughOffset(graphics) + 0.5), x - startX, (int) (fmcInfo
						.getStrikethroughThickness(graphics) + 0.5));
			}
			...
			if (underlineColor != null) { // draw underline
				...
				graphics.fillRect(startX, startY + (int) (fmcInfo.getUnderlineOffset(graphics) + 1.5), x - startX, (int) (fmcInfo
						.getUnderlineThickness(graphics) + 0.5));
			}
		...
		public void drawChars(int offset, int length, int width, Color strikeThroughColor, Color underlineColor) {
			// Check if strike-through and underline colors match the current
			// ones
			if ((strikeThroughColor == null && this.strikeThroughColor != null)
					|| (strikeThroughColor != null && !strikeThroughColor.equals(this.strikeThroughColor))
					|| (underlineColor == null && this.underlineColor != null) || (underlineColor != null && !underlineColor.equals(this.underlineColor))) {
				flush();
			}
	...
	static final class PrintDG extends SimpleDG {
		 ...
		 * 
		 * @param container
		 *            print container to which the tokens are added.
		 */
