...
/**
 * The last several jumps in either the current file or across several files is
...
public class JumpList {
	...
	/**
	 * Maximum size to which the list will be shrinked if it exceeds the
	 * THRESHOLD_SIZE.
	 */
	private static final int MAX_SIZE = 50;

	/**
	 * Reaching this count means that the size should be checked and possibly
	 * shrinked to the MAX_SIZE.
	 */
	...
	/**
	 * @param c
	 *            current component. It's used to compare the current component
	 *            and position with those stored in the entries.
	 */
	public static void jumpPrev(JTextComponent c) {
		...
		if (currentEntry != null) {
			while (true) {
				...
				if (currentEntry.prev != null) { // must check not to end up
													// with null
	...
	public static void jumpNext(JTextComponent c) {
		...
		if (currentEntry != null) {
			while (true) {
				...
				if (currentEntry.next != null) { // must check not to end up
													// with null
	 ...
	 * @param c
	 *            current component. It's used to compare the current component
	 *            to those stored in the jump list entries.
	 */
	...
	public static String dump() {
		...
		if (e != null) {
			...
			while (e != null) {
				...
				String docStr = (comp != null) ? (String) comp.getDocument().getProperty(Document.TitleProperty) : "<Invalid document>"; // NOI18N
