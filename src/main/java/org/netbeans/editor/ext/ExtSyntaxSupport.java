...
public class ExtSyntaxSupport extends SyntaxSupport {
	...
	/**
	 * Listens for the changes on the document. Children can override the
	 * documentModified() method to perform some processing.
	 */
	...
	/**
	 * Get the chain of the tokens for the given block of text. The returned
	 ...
	 * reaching the first chain item and calling <tt>getPrevious()</tt> on it.
	 ...
	 * moment. Otherwise this call throws <tt>IllegalStateException</tt>.
	 * 
	 * @param startOffset
	 *            starting position of the block
	 * @param endOffset
	 *            ending position of the block
	 * @return the first item of the token-item chain or null if there are no
	 *         tokens in the given area or the area is so small that it lays
	 *         inside one token. To prevent this provide the area that spans a
	 *         new-line.
	 */
	...
	/**
	 * Get the bracket finder that will search for the matching bracket or null
	...
	/**
	 * Find matching bracket or more generally block that matches with the
	 * current position.
	 * 
	 * @param offset
	 *            position of the starting bracket
	 * @param simple
	 *            whether the search should skip comment and possibly other
	 *            areas. This can be useful when the speed is critical, because
	 *            the simple search is faster.
	 * @return array of integers containing starting and ending position of the
	 *         block in the document. Null is returned if there's no matching
	 *         block.
	 */
	...
	/**
	 * Get the array of token IDs that should be skipped when searching for
	 ...
	 * 
	 * @param offset
	 *            position at which the token should be returned
	 * @return token-id of the token at the requested position. If there's no
	 *         more tokens in the text, the <tt>Syntax.INVALID</tt> is
	 *         returned.
	 */
	...
	/**
	 * Get the array of token IDs that denote the comments. Returns empty array
	 * by default.
	 */
	 ...
	 * @param startPos
	 *            starting position of the searched document area
	 * @param endPos
	 *            ending position of the searched document area
	 */
	...
	/**
	 * Find the type of the variable. The default behavior is to first search
	 ...
	 * 
	 * @return it returns Object to enable the custom implementations to return
	...
	/**
	 * Get the start position of the method or the area where the declaration
	 * can start.
	 */
	...
	/**
	 * Find either the local or global declaration position. First try the local
	...
	/**
	 * Get the position of the global declaration of a given variable. By
	...
	/**
	 * Token processor extended to get declaration position of the given
	 * variable.
	 */
	...
	/**
	 * Finder for the matching bracket. It gets the original bracket char and
	 * searches for the appropriate matching bracket character.
	 */
	public class BracketFinder extends FinderFactory.GenericFinder {
		...
		/**
		 * Will it be a forward finder +1 or backward finder -1 or 0 when the
		 * given character is not bracket character.
		 */
		 ...
		 * If so assign the matchChar and moveCount variables.
		 */
	...
	final class TokenItemTP implements TokenProcessor {
		...
		/**
		 * Target position corresponding to the begining of the token that is
		 * already chained if searching for backward tokens, or, the last token
		 * that should be scanned if searching in forward direction.
		 */
		int targetOffset;

		TokenItemTP() {
			fwdBatchLineCnt = bwdBatchLineCnt = ((Integer) getDocument().getProperty(SettingsNames.LINE_BATCH_SIZE)).intValue();
			// Start listening on document changes
			docL = new DocumentListener() {
				public void insertUpdate(DocumentEvent evt) {
					if (lastItem != null && evt.getOffset() < lastItem.getOffset() + lastItem.getImage().length()) {
						docModified = true;
		...
		public boolean token(TokenID tokenID, TokenContextPath tokenContextPath, int tokenBufferOffset, int tokenLength) {
			if (bufferStartPos + tokenBufferOffset >= targetOffset) { // stop
																		// scanning
				return false;
			}

			lastItem = new Item(tokenID, tokenContextPath, bufferStartPos + tokenBufferOffset, new String(buffer, tokenBufferOffset, tokenLength), lastItem);
		...
		Item getPreviousChunk(Item i) {
			...
			try {
				...
				lastItem = lit;
				if (firstItem == null) { // restore in case of no token or
											// crash
					firstItem = fit;
				}
			}
