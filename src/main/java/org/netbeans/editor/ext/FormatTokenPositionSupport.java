...
/**
 * Support class for mapping the token-positions to the tokens and providing
...
class FormatTokenPositionSupport {
	 ...
	 * @param offset
	 *            offset inside the token at which the position is being
	 *            created.
	 */
	...
	/**
	 * Notify that the previous token was created with the appropriate text
	 * taken from the start of this token. It's now necessary to split the marks
	 * according
	 * 
	 * @param token
	 *            token that was split
	 * @param startLength
	 *            initial length of the token-text that was cut and inserted
	 *            into the previous token in the chain.
	 */
	...
	synchronized void tokenTextInsert(TokenItem token, int offset, int length) {
		...
		for (int i = 0; i < len; i++) {
			ExtTokenPosition etp = (ExtTokenPosition) posList.get(i);
			if ((etp.bias == Position.Bias.Backward) ? (etp.offset > offset) : (etp.offset >= offset)) {
				etp.offset += length;
			}
		}
	...
	synchronized void tokenTextRemove(TokenItem token, int offset, int length) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (etp.offset >= newLen) { // need to move to begining of next
										// token
	...
	/**
	 * Add the save-set to the registry and perform the checking whether the
	 * offsets are OK.
	 */
	synchronized void addSaveSet(int baseOffset, int writtenLen, int[] offsets, Position.Bias[] biases) {
		...
		for (int i = 0; i < offsets.length; i++) {
			if (offsets[i] < 0 || offsets[i] > writtenLen) {
				throw new IllegalArgumentException("Invalid save-offset=" + offsets[i] + " at index=" + i // NOI18N
			...
			for (int i = 0; i < len; i++) {
				if (noText) {
					...
					while (token != null) {
						...
						else if ((offset > token.getSaveOffset() + token.getImage().length()) || token.getImage().length() == 0) {
							token = (FormatWriter.FormatTokenItem) token.getNext();

						}
						else { // the right token
							curSet.positions[i] = getTokenPosition(token, offset - token.getSaveOffset(), curSet.biases[i]);
	...
	synchronized void updateSaveSets(FormatTokenPosition formatStartPosition) {
		...
		if (formatStartPosition.getToken() != null) {
			endOffset = ((FormatWriter.FormatTokenItem) formatWriter.getLastToken()).getSaveOffset() + formatWriter.getLastToken().getImage().length();
		}
		...
		while (curSet != null) {
			...
			for (int i = 0; i < len; i++) {
				...
				if (token == null) {
					...
					curSet.offsets[i] = token.getSaveOffset() + curSet.positions[i].getOffset();
				}
			}
		}
	}
	...
	/**
	 * Number the tokens so that they are OK for finding out the offsets.
	 */
	private void updateSaveOffsets(FormatTokenPosition formatStartPosition) {
		if (firstSet != null) { // it has only sense if there are any save-sets
			FormatWriter.FormatTokenItem ti = (FormatWriter.FormatTokenItem) formatStartPosition.getToken();
	...
	/**
	 * Implementation of the extended-token-position that allows modification of
	 * its token and offset fields.
	 */
	class ExtTokenPosition implements FormatTokenPosition {
		...
		public boolean equals(Object o, boolean ignoreBias) {
			if (o instanceof FormatTokenPosition) {
				...
				return token == tp.getToken() && offset == tp.getOffset() && (ignoreBias || bias == tp.getBias());
			}
	...
	/**
	 * Class holding the info about the set of the offsets to save during the
	 * formatting.
	 */
