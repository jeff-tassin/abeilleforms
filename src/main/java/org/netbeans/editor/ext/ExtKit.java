...
public class ExtKit extends BaseKit {
	...
	/**
	 * This action is searched and executed when the popup menu should be
	 * displayed to build the popup menu.
	 */
	...
	/**
	 * This action is searched and executed when the tool-tip should be
	 * displayed by tool-tip support to build the tool-tip.
	 */
	public static final String buildToolTipAction = "build-tool-tip"; // NOI18N

	/**
	 * Open find dialog action - this action is defined in view package, but its
	 * name is defined here for clarity
	 */
	...
	/**
	 * Open goto dialog action - this action is defined in view package, but its
	 * name is defined here for clarity
	 */
	...
	protected Action[] createActions() {
		Action[] extActions = new Action[] { new BuildPopupMenuAction(), new ShowPopupMenuAction(), new BuildToolTipAction(), new FindAction(),
				new ReplaceAction(), new GotoAction(),
				...
				// new CodeSelectAction(), // the actionPerformed is empty and
				// so I'm removing the action from the list
				new EscapeAction(), new ExtDefaultKeyTypedAction(), new ExtInsertTabAction(), new ExtInsertBreakAction(), new ExtUpAction(upAction, false),
				new ExtPageUpAction(pageUpAction, false), new ExtDownAction(downAction, false), new ExtPageDownAction(pageDownAction, false),
				new ExtBeginLineAction(beginLineAction, false), new ExtBeginLineAction(selectionBeginLineAction, true),
				new ExtEndLineAction(endLineAction, false), new ExtEndLineAction(selectionEndLineAction, true), new CompletionShowAction(),
				new CompletionHideAction(), new ShiftInsertBreakAction() };
	...
	public static class BuildPopupMenuAction extends BaseAction {
		...
		/**
		 * Add the action to the popup menu. This method is called for each
		 * action-name found in the action-name-list. It should add the
		 * appopriate menu item to the popup menu.
		 * 
		 * @param target
		 *            target component for which the menu is being constructed.
		 * @param popupMenu
		 *            popup menu to which this method should add the item
		 *            corresponding to the action-name.
		 * @param actionName
		 *            name of the action to add. The real action can be
		 *            retrieved from the kit by calling
		 *            <tt>getActionByName()</tt>.
		 */
	...
	public static class GotoDeclarationAction extends BaseAction {
		...
		public GotoDeclarationAction() {
			super(gotoDeclarationAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | SAVE_POSITION);
		}
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			if (target != null) {
				gotoDeclaration(target); // try to go to the declaration
											// position
			}
		}
	}
	...
	public static class ToggleCaseIdentifierBeginAction extends BaseAction {
		...
		ToggleCaseIdentifierBeginAction() {
			super(toggleCaseIdentifierBeginAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	public static class MatchBraceAction extends BaseAction {
		...
		public MatchBraceAction(String name, boolean select) {
			super(name, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET);
			this.select = select;
		}
	...
	public static class CodeSelectAction extends BaseAction {
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			/*
			 * if (target != null) { BaseDocument doc =
			 * (BaseDocument)target.getDocument(); SyntaxSupport sup =
			 * doc.getSyntaxSupport(); Caret caret = target.getCaret(); try {
			 * int bracketPos = sup.findUnmatchedBracket(caret.getDot(),
			 * sup.getRightBrackets()); if (bracketPos >= 0) {
			 * caret.setDot(bracketPos); while (true) { int bolPos =
			 * Utilities.getRowStart(doc, bracketPos); boolean isWSC =
			 * sup.isCommentOrWhitespace(bolPos, bracketPos); if (isWSC) { //
			 * get previous line end
			 *  } } } } catch (BadLocationException e) {
			 * target.getToolkit().beep(); } }
			 */
		}
	...
	/**
	 * Prefix maker adds the prefix before the identifier under cursor. The
	 * prefix is not added if it's already present. The prefix to be added is
	 * specified in the constructor of the action together with the prefix
	 * group. If there's already any prefix from the prefix group at the
	 * begining of the identifier, that prefix is replaced by the actual prefix.
	 */
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			if (target != null) {
				...
				try {
					...
					// Handle the case we already have the work done - e.g. if
					// we got called over 'getValue'
					if (identifier.startsWith(prefix) && Character.isUpperCase(identifier.charAt(prefix.length())))
						return;

					// Handle the case we have other type of known xEr: eg
					// isRunning -> getRunning
					for (int i = 0; i < prefixGroup.length; i++) {
						String actPref = prefixGroup[i];
						if (identifier.startsWith(actPref) && identifier.length() > actPref.length()
			...
			if (target != null) {
				...
				try {
					if (caret.isSelectionVisible()) {
						...
						doc.insertString(Utilities.getRowStart(doc, target.getSelectionStart()), lineCommentString, null); // NOI18N
					}
				} catch (BadLocationException e) {
					target.getToolkit().beep();
				}
			...
			if (target != null) {
				...
				try {
					if (caret.isSelectionVisible()) {
						...
						if (Utilities.getRowEnd(doc, pos) - pos >= 2 && doc.getText(pos, 2).equals(lineCommentString) // NOI18N
						) {
							doc.remove(pos, 2);
						}
					}
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			...
			if (cmd != null && cmd.equals(" ") && (mod == ActionEvent.CTRL_MASK)) { // NOI18N
				...
				if (caret instanceof ExtCaret) {
					((ExtCaret) caret).requestMatchBraceUpdateSync(); // synced
																		// bracket
																		// update
				}
			...
			if ((target != null) && (evt != null)) {
				if ((cmd != null) && (cmd.length() == 1) && ((mod & ActionEvent.ALT_MASK) == 0 && (mod & ActionEvent.CTRL_MASK) == 0)) {
					// Check whether char that should reindent the line was
					// inserted
				...
				if (f instanceof ExtFormatter) {
					...
					if (fmtBlk != null) {
						try {
							...
							// this was the of #18922, that causes the bug
							// #20198
							// ef.reformat(doc, fmtBlk[0], fmtBlk[1]);

							// bugfix of the bug #20198. Bug #18922 is fixed too
							// as well as #6968
	...
	public static class ExtUpAction extends UpAction {

		static final long serialVersionUID = 9039435547323841544L;

		ExtUpAction(String nm, boolean select) {
			super(nm, select);
		}
	...
	public static class ExtDownAction extends DownAction {

		static final long serialVersionUID = 5728488498924841815L;

		ExtDownAction(String nm, boolean select) {
			super(nm, select);
		}
	...
	public static class ExtPageUpAction extends PageUpAction {

		static final long serialVersionUID = 4336328849128594564L;

		ExtPageUpAction(String nm, boolean select) {
			super(nm, select);
		}
	...
	public static class ExtPageDownAction extends PageDownAction {

		static final long serialVersionUID = -2373608546477823838L;

		ExtPageDownAction(String nm, boolean select) {
			super(nm, select);
		}
	...
	public static class ExtBeginLineAction extends BeginLineAction {

		static final long serialVersionUID = 1226924439847693361L;

		public ExtBeginLineAction(String nm, boolean select) {
			super(nm, select);
		}
	...
	public static class ExtEndLineAction extends EndLineAction {

		static final long serialVersionUID = -3703912765497771472L;

		public ExtEndLineAction(String nm, boolean select) {
			super(nm, select);
		}
	...
	public static class ShiftInsertBreakAction extends BaseAction {
		...
		public ShiftInsertBreakAction() {
			super(shiftInsertBreakAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET);
		}
