...
public class ExtFinderFactory {
	...
	/**
	 * Finder that collects the whole lines and calls the <tt>lineFound()</tt>
	 * method that can do a local find. !!! Udelat to poradne i s vice bufferama
	 */
	public static abstract class LineFwdFinder extends FinderFactory.AbstractFinder {
		...
		public int find(int bufferStartPos, char buffer[], int offset1, int offset2, int reqPos, int limitPos) {
			int offset = reqPos - bufferStartPos; // !!! Udelat poradne s
													// moznosti vice bufferu
			while (true) {
				...
				int lineOffset = lineFound(buffer, offset, lfOffset, Math.max(origStartPos - bufferStartPos, offset), Math.min(origLimitPos - bufferStartPos,
						lfOffset));
				if (lineOffset >= 0) {
					found = true;
		...
		/**
		 * Line was found and is present in the given buffer. The given buffer
		 * is either the original buffer passed to the <tt>find()</tt> or
		 * constructed buffer if the line is at the border of the previous and
		 * next buffer.
		 * 
		 * @return non-negative number means the target string was found and the
		 *         returned number is offset on the line where the string was
		 *         found. Negative number means the target string was not found
		 *         on the line and the search will continue with the next line.
		 */
	...
	public static abstract class LineBwdFinder extends FinderFactory.AbstractFinder {
		...
		/** find function that must be defined by descendant */
		public int find(int bufferStartPos, char buffer[], int offset1, int offset2, int reqPos, int limitPos) {
			int offset = reqPos - bufferStartPos + 1; // !!! Udelat poradne s
														// moznosti vice bufferu
			while (true) {
				...
				int lineOffset = lineFound(buffer, lfOffsetP1, offset, Math.max(origLimitPos - bufferStartPos, lfOffsetP1), Math.min(origStartPos
