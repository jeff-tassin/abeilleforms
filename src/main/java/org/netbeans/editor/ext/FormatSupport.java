...
/**
 * Format support presents a set of operations over the format-writer that is
 * specific for the given set of formatting layers. It presents the way how to
 * extend the low level methods offered by the format-writer. In general there
 * can be more format-layers that use one type of the format-support.
 * 
...
public class FormatSupport {
	...
	public int getTabSize() {
		...
		return (doc instanceof BaseDocument) ? ((BaseDocument) doc).getTabSize() : formatWriter.getFormatter().getTabSize();
	}

	public int getShiftWidth() {
		Document doc = formatWriter.getDocument();
		return (doc instanceof BaseDocument) ? ((BaseDocument) doc).getShiftWidth() : formatWriter.getFormatter().getShiftWidth();
	}
	...
	public boolean getSettingBoolean(String settingName, boolean defaultValue) {
		return ((Boolean) getSettingValue(settingName, (defaultValue ? Boolean.TRUE : Boolean.FALSE))).booleanValue();
	}
	...
	public TokenItem insertToken(TokenItem beforeToken, TokenID tokenID, TokenContextPath tokenContextPath, String tokenImage) {
		return formatWriter.insertToken(beforeToken, tokenID, tokenContextPath, tokenImage);
	}
	...
	public void insertSpaces(TokenItem beforeToken, int spaceCount) {
		...
		insertToken(beforeToken, whitespaceTokenID, null, new String(Analyzer.getSpacesBuffer(spaceCount), 0, spaceCount));
	}
	 ...
	 * 
	 * @param token
	 *            token for which the token-position is being created.
	 * @param offset
	 *            offset inside the token.
	 */
	...
	/**
	 * Get the next position of the position given by parameters. It can be
	 * either just offset increasing but it can be movement to the next token
	 * for the token boundary.
	 * 
	...
	/**
	 * Get the previous position of the position given by parameters. It can be
	 * either just offset decreasing but it can be movement to the previous
	 * token for the token boundary.
	 * 
	...
	public FormatTokenPosition getPreviousPosition(TokenItem token, int offset, Position.Bias bias) {
		...
		if (token == null) { // end of chain
			...
			if (lastToken != null) { // regular last token
				ret = getPosition(lastToken, lastToken.getImage().length() - 1, Position.Bias.Forward);
			}
			...
			if (offset < 0) {
				...
				if (token != null) { // was first pos in first token
					ret = getPosition(token, token.getImage().length() - 1, Position.Bias.Forward);
				}

			}
			else { // still inside token
				ret = getPosition(token, offset, Position.Bias.Forward);
			}
		}
	...
	/**
	 * Get the token-position preceeding the given one. Use the same bias like
	 * the given position has.
	 */
	public FormatTokenPosition getPreviousPosition(FormatTokenPosition pos) {
		return getPreviousPosition(pos.getToken(), pos.getOffset(), pos.getBias());
	}
	 ...
	 * 
	 * @param bias
	 *            bias that the returned position will have.
	 */
	...
	/**
	 * Get the next successive token-position after the given one. Use the same
	 * bias like the given position has.
	 */
	public FormatTokenPosition getNextPosition(FormatTokenPosition pos) {
		return getNextPosition(pos.getToken(), pos.getOffset(), pos.getBias());
	}
	...
	/**
	 * Whether the given token can be replaced or not. It's identical to whether
	 ...
	 * @param tokenImage
	 *            token-text of the new token-item
	 */
	...
	/**
	 * Compare token-id of the compare-token with the given token-id. Token text
	 * and token-context-path are ignored in comparison.
	 * 
	 * @param compareToken
	 *            token to compare
	 * @param withTokenID
	 ...
	 * @param withTokenID
	 *            token-id with which the token's token-id is compared.
	 * @param withTokenContextPath
	 *            token-context-path to which the token's token-context-path is
	 *            compared.
	 * @return true if the token-ids match, false otherwise
	 */
	...
	/**
	 * Compare token-id of the compare-token with the given token-id and given
	 * token-text.
	 * 
	 * @param compareToken
	 *            token to compare
	 * @param withTokenID
	 *            token-id with which the token's token-id is compared. It can
	 *            be null in which case the token-id is ignored from comparison.
	 * @param withTokenContextPath
	 *            token-context-path to which the token's token-context-path is
	 *            compared. It can be null in which case the token-context-path
	 *            is ignored from comparison.
	 * @param withTokenImage
	 *            token-text with which the token's token-text is compared. It
	 *            can be null in which case the token-text is ignored from
	 *            comparison.
	...
	public boolean tokenEquals(TokenItem compareToken, TokenID withTokenID, TokenContextPath withTokenContextPath, String withTokenImage) {
		return (withTokenID == null || compareToken.getTokenID() == withTokenID)
				&& (withTokenContextPath == null || compareToken.getTokenContextPath() == withTokenContextPath)
				&& (withTokenImage == null || compareToken.getImage().equals(withTokenImage));
	}

	/**
	 * Decide whether the character at the given offset in the given token is
	 * whitespace.
	 */
	 ...
	 * character in backward direction and returns the position of the character
	 * following the new-line character or the first character of the first
	 * token in the chain.
	 * 
	 * @param pos
	 *            any token-position on the line.
	 */
	 ...
	 * character and returns the position of it or the position after the last
	 * character of the last token in the chain.
	 * 
	 * @param pos
	 *            any token-position on the line.
	 */
	...
	/**
	 * Return the first non-whitespace character on the line or null if there is
	...
	/**
	 * Return the ending whitespace on the line or null if there's no such token
	 * on the given line.
	 */
	...
	/**
	 * Get the first EOL in backward direction. The current position is ignored
	 * by the search.
	 * 
	 * @return first EOL in backward direction or null if there is no such
	 *         token.
	 */
	 ...
	 * 
	 * @param pos
	 *            starting token-position that is ignored by the search so it
	 *            can be even EOL.
	 * @return first EOL token-position in the forward direction or null if
	...
	/**
	 * Check whether there are no tokens except the ending EOL on the given
	 * line.
	 * 
	 * @param pos
	 ...
	 * 
	 * @param token
	 *            any token on the line. It doesn't have to be the first one.
	 */
	...
	/**
	 * Get the column-offset of the tokenItem on its line. The tabs are expanded
	 * according to the tab-size.
	 */
	 ...
	 * 
	 * @param startPosition
	 *            position at which the search starts. For the backward search
	 *            the character right at startPosition is not considered as part
	 *            of the search.
	 * @param limitPosition
	 *            the token where the search will be broken reporting that
	 *            nothing was found. It can be null to search till the end or
	 *            begining of the chain (depending on direction). For forward
	 *            search the char at the limitPosition is not considered to be
	 *            part of search, but for backward search it is.
	 * @param stopOnEOL
	 *            whether stop and return EOL position or continue search if EOL
	 *            token is found.
	 * @param backward
	...
	/**
	 * Get the token-id that should be assigned to the token that consists of
	 * the indentation whitespace only. This method should be overriden in the
	 * descendants.
	 */
	...
	/**
	 * Get the valid whitespace token-id by calling
	 * <tt>getWhitespaceTokenID()</tt>. Throw <tt>IllegalStateException</tt>
	 * if the whitespace-token-id is null.
	 */
	...
	/**
	 * Get the token-context-path that should be assigned to the token that
	 * consists of the indentation whitespace only. This method should be
	 * overriden in the descendants.
	 */
	...
	/**
	 * Get the valid whitespace token-context-path by calling
	 * <tt>getWhitespaceTokenContextPath()</tt>. Throw
	 * <tt>IllegalStateException</tt> if the whitespace-token-id is null.
	 */
	 ...
	 * This method should be overriden in the descendants.
	 */
	 ...
	 * 
	 * @param formatTokenPosition
	 *            any position on the line. It doesn't have to be the first one.
	 * @param zeroForWSLine
	 *            If set to true the method will return zero in case the line
	 *            consist of whitespace only. If false the method will return
	 *            the indentation even for whitespace lines.
	 */
	...
	/**
	 * Change the indentation of the line. This method should be always called
	 * for all the lines because it ensures that the indentation will contain
	 * exactly the characters from the indentation string.
	 * 
	 * @param pos
	 *            any position on the line being checked.
	 * @param indent
	...
	public FormatTokenPosition changeLineIndent(FormatTokenPosition pos, int indent) {
		...
		if (token == null) { // last line is empty, append the indent string
			if (indentString.length() > 0) {
				token = insertToken(null, getValidWhitespaceTokenID(), getValidWhitespaceTokenContextPath(), indentString);
			}
		...
		while (true) {
			...
			while (indentStringInd < indentStringLen && offset < textLen) {
				if (indentString.charAt(indentStringInd) != text.charAt(offset)) {
					if (canModifyWhitespace(token)) {
						...
						offset += indentStringLen - indentStringInd; // skip
																		// WS
						...
						if (isWhitespace(token, offset) || offset > 0) {
							throw new IllegalStateException("Cannot modify token=" + token); // NOI18N

						}
						else { // nonWS token at begining, will insert WS
							insertToken(token, getValidWhitespaceTokenID(), getValidWhitespaceTokenContextPath(), indentString.substring(indentStringInd));
							return getPosition(token, 0);
						}
					}
			...
			if (indentStringInd < indentStringLen) { // move to next token
				...
				if (token == null) { // was last token, insert WS token
					token = insertToken(null, getValidWhitespaceTokenID(), getValidWhitespaceTokenContextPath(), indentString.substring(indentStringInd));
					return getPosition(token, 0);
