...
/**
 * Generator of code used for matching the keywords or more generally some group
 * of words.
 * 
...
public class KeywordMatchGenerator {
	...
	private static final String USAGE = "Usage: java org.netbeans.editor.ext.KeywordMatchGenerator [options]" // NOI18N
	...
	private void parseKeywords(String s) {
		...
		try {
			while (true) {
				...
				keyList.add(key);
				kwdConstants.put(key, getKwdConstantPrefix() + (upperCaseKeyConstants() ? key.toUpperCase() : key));
			}
	 ...
	 * called with the full range of keywords and offset equal to zero. It
	 * recursively calls itself to scan the subgroups.
	 * 
	 * @param indFrom
	 *            index in kwds[] where the subgroup of keywords starts
	 * @pararm indTo index in kwds[] where the subgroup of keywords ends
	 * @param offset
	 *            current horizontal offset. It's incremented as the subgroups
	 *            are recognized. All the characters prior to offset index are
	 *            the same in all keywords in the group.
	 */
	private void scan(int indFrom, int indTo, int offset, int indent, int minKwdLen) {
		// System.out.println(">>>DEBUG<<< indFrom=" + indFrom + ", indTo=" +
		// indTo + ", offset=" + offset + ", indent=" + indent + ", minKwdLen="+
		// minKwdLen); // NOI18N
		int maxLen = 0;
		...
		do {
			...
			// System.out.println(">>>DEBUG<<< while(): minLen=" + minLen + ",
			// minKwdLen=" + minKwdLen); // NOI18N
			if (minLen > minKwdLen) {
			...
			// System.out.println(">>>DEBUG<<< minLen=" + minLen + ", maxLen=" +
			// maxLen + ", same=" + same); // NOI18N
			...
			if (offset == kwds[indFrom].length()) {
				...
				indFrom++; // increase starting index as first keyword already
							// matched
				if (offset >= minLen) {
					minLen = offset + 1;
				}
		...
		if (offset < maxLen) {
			...
			while (i < indTo) {
				...
				while (subGroupEndInd < indTo && kwds[subGroupEndInd].length() > offset && kwds[subGroupEndInd].charAt(offset) == actChar) {
					subGroupEndInd++;
				}

				if (subGroupEndInd > i + 1) { // more than one keyword in
												// subgroup
