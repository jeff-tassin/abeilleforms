...
public class ExtCaret extends BaseCaret {
	...
	/**
	 * Signal that the next matching brace update will be immediate without
	 * waiting for the brace timer to fire the action.
	 */
	...
	protected void modelChanged(BaseDocument oldDoc, BaseDocument newDoc) {
		// Fix for #7108
		braceMarksValid = false; // brace marks are out of date - new
									// document
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (c != null) {
			...
			highlightRowColoring = editorUI.getColoring(ExtSettingsNames.HIGHLIGHT_CARET_ROW_COLORING);
			highlightBraceColoring = editorUI.getColoring(ExtSettingsNames.HIGHLIGHT_MATCH_BRACE_COLORING);

			// Handle highlight row
			boolean oldHighlightRow = highlightRow;
			highlightRow = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.HIGHLIGHT_CARET_ROW, ExtSettingsDefaults.defaultHighlightCaretRow);
			...
			highlightBrace = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.HIGHLIGHT_MATCH_BRACE, ExtSettingsDefaults.defaultHighlightMatchBrace);
			int highlightBraceDelay = SettingsUtil.getInteger(kitClass, ExtSettingsNames.HIGHLIGHT_MATCH_BRACE_DELAY,
			...
			if (highlightBrace) {
				if (highlightBraceDelay > 0) {
					...
					braceTimerListener = new ActionListener() {
			...
			simpleMatchBrace = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.CARET_SIMPLE_MATCH_BRACE, ExtSettingsDefaults.defaultCaretSimpleMatchBrace);

			popupMenuEnabled = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.POPUP_MENU_ENABLED, true);
		}
	}
	...
	/**
	 * Update the matching brace of the caret. The document is read-locked while
	...
	protected void updateMatchBrace() {
		...
		if (c != null && highlightBrace) {
			try {
				...
				int dotPos = getDot();
				boolean madeValid = false; // whether brace marks display were
											// validated
				if (dotPos > 0) {
					int[] matchBlk = ((ExtSyntaxSupport) doc.getSyntaxSupport()).findMatchingBlock(dotPos - 1, simpleMatchBrace);
					if (matchBlk != null) {
						if (highlightBraceStartMark != null) {
							...
							if (markStartPos != matchBlk[0] || markEndPos != matchBlk[1]) {
								...
								if (!braceMarksValid) { // was not valid, must
														// repaint
									editorUI.repaintBlock(matchBlk[0], matchBlk[1]);
								}
							}
							...
							highlightBraceStartMark = new MarkFactory.DrawMark(HIGHLIGHT_BRACE_LAYER_NAME, editorUI);
							highlightBraceEndMark = new MarkFactory.DrawMark(HIGHLIGHT_BRACE_LAYER_NAME, editorUI);
				...
				if (!madeValid) {
					if (braceMarksValid) {
						...
						editorUI.repaintBlock(highlightBraceStartMark.getOffset(), highlightBraceEndMark.getOffset());
					}
				}
	...
	private static boolean isRightMouseButton(MouseEvent e) {
		...
		return (m & java.awt.event.InputEvent.BUTTON1_MASK) == 0
				&& (m & (java.awt.event.InputEvent.BUTTON2_MASK | java.awt.event.InputEvent.BUTTON3_MASK)) != 0;
	}
	...
	public void mouseReleased(MouseEvent evt) {
		...
		if (c != null) {
			...
			if (isRightMouseButton(evt) && popupMenuEnabled) {
				ExtUtilities.getExtEditorUI(c).showPopupMenu(evt.getX(), evt.getY());
			}
			else {
