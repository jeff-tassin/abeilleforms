...
/**
 * Extended format-support offers comment-token support, token-and-text
...
public class ExtFormatSupport extends FormatSupport {
	 ...
	 * 
	 * @param fromPosition
	 *            the position from which to start counting. If it's EOL, it's
	 *            counted.
	 * @param toPosition
	 *            the ending position. If it points at EOL, it's ignored from
	 *            the total count. It is necessary for the second position to
	...
	/**
	 * Is the given token a comment token? By default it returns false but it
	 ...
	 * 
	 * @param startPosition
	 *            position from which the search starts. For the backward search
	 *            the character right at startPosition is not considered as part
	 *            of the search.
	 * @param limitPosition
	 *            position where the search will be broken reporting that
	 *            nothing was found. It can be null to search till the end or
	 *            begining of the chain (depending on direction).
	 * @param stopOnEOL
	 *            whether stop and return EOL token or continue search if EOL
	 *            token is found.
	 * @param backward
	 *            whether search in backward direction.
	 * @return first non-whitespace token or EOL or null if all the tokens till
	 *         the begining of the chain are whitespaces.
	 */
	...
	/**
	 * Get the start of the area of line where there is only whitespace or
	 * comment till the end of the line.
	 * 
	 * @param pos
	 *            any position on the line. Return null if there's no such area.
	 */
	...
	/**
	 * Insert the token that has token-id containing image, so additional text
	 * is not necessary.
	 */
	public TokenItem insertImageToken(TokenItem beforeToken, ImageTokenID tokenID, TokenContextPath tokenContextPath) {
		return super.insertToken(beforeToken, tokenID, tokenContextPath, tokenID.getImage());
	}
	 ...
	 * 
	 * @param startToken
	 *            token from which to start searching. For backward search this
	 *            token is excluded from the search.
	 * @param limitToken
	 *            the token where the search will be broken reporting that
	 *            nothing was found. It can be null to search till the end or
	 *            begining of the chain (depending on direction). For forward
	 *            search this token is not considered to be part of search, but
	 *            for backward search it is.
	 * @param tokenID
	 *            token-id to be searched. If null the token-id of the tokens
	 *            inspected will be ignored.
	 * @param tokenImage
	 *            text of the token to find. If null the text of the tokens
	 *            inspected will be ignored.
	 * @param backward
	 *            true for searching in backward direction or false to serach in
	 *            forward direction.
	 * @return return the matching token or null if nothing was found
	 */
	public TokenItem findToken(TokenItem startToken, TokenItem limitToken, TokenID tokenID, TokenContextPath tokenContextPath, String tokenImage,
			boolean backward) {

		if (backward) { // go to the previous token for the backward search
			if (startToken != null && startToken == limitToken) { // empty
																	// search
				return null;
			}
	 ...
	 * direction. This is similair to <tt>findImportant()</tt> but it operates
	 * over the tokens.
	 * 
	 * @param startToken
	 *            token from which to start searching. For backward search this
	...
	/**
	 * This method can be used to find a matching brace token. Both the token-id
	 * and token-text are used for comparison of the starting token.
	 * 
	 * @param startToken
	 *            token from which to start. It cannot be null. For backward
	 *            search this token is ignored and the previous one is used.
	 * @param limitToken
	 *            the token where the search will be broken reporting that
	 *            nothing was found. It can be null to search till the end or
	 *            begining of the chain (depending on direction). For forward
	 ...
	 *            for backward search it is.
	 * @param matchTokenID
	 *            matching token-id for the start token.
	 * @param matchTokenImage
	 *            matching token-text for the start token.
	 * @param backward
	 *            true for searching in backward direction or false to serach in
	 *            forward direction.
	 */
	public TokenItem findMatchingToken(TokenItem startToken, TokenItem limitToken, TokenID matchTokenID, String matchTokenImage, boolean backward) {
		...
		while (token != null && token != limitToken) {
			if (tokenEquals(token, matchTokenID, startTokenContextPath, matchTokenImage)) {
				if (depth-- == 0) {
					return token;
				}

			}
			else if (tokenEquals(token, startTokenID, startTokenContextPath, startText)) {
				depth++;
			}
	...
	public TokenItem findMatchingToken(TokenItem startToken, TokenItem limitToken, ImageTokenID matchTokenID, boolean backward) {
		return findMatchingToken(startToken, limitToken, matchTokenID, matchTokenID.getImage(), backward);
	}

	/**
	 * Search for any of the image tokens from the given array and return if the
	 * token matches any item from the array. The index of the item from the
	 * array that matched can be found by calling <tt>getIndex()</tt> method.
	 * It is suitable mainly for the image-token-ids.
	 * 
	 * @param startToken
	 *            token from which to start. For backward search this token is
	 ...
	 *            for backward search it is.
	 * @param tokenIDArray
	 *            array of the token-ids for which to search.
	 * @param tokenContextPath
	 *            context path that the found token must have. It can be null.
	 * @param backward
	 *            true for searching in backward direction or false to serach in
	 *            forward direction.
	 */
	...
	/**
	 * Get the index of the token in the given token-id-and-text array or -1 if
	 ...
	 * 
	 * @param pos
	 *            position on the line to be checked.
	...
	/**
	 * Get the character at the given position. The caller must care about not
