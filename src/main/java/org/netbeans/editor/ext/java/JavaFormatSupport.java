...
public class JavaFormatSupport extends ExtFormatSupport {
	...
	public TokenItem findSwitch(TokenItem caseToken) {
		if (caseToken == null
				|| (!tokenEquals(caseToken, JavaTokenContext.CASE, tokenContextPath) && !tokenEquals(caseToken, JavaTokenContext.DEFAULT, tokenContextPath))) {
				...
				if (stmt == null) {
					...
					default:
						...
						if (stmt == null) {
							...
							stmt = findStatement(token); // search for
															// delimiter
	...
	public FormatTokenPosition findLineFirstNonWhitespaceAndNonLeftBrace(FormatTokenPosition pos) {
		...
		TokenItem rbmt = findMatchingToken(ftp.getToken(), eolp != null ? eolp.getToken() : null, JavaTokenContext.RBRACE, false);
		if (rbmt != null)
			return ftp;
