...
public class JavaDrawLayerFactory {
	...
	public static class JavaLayer extends DrawLayer.AbstractLayer {
		...
		protected Coloring getMethodColoring(DrawContext ctx) {
			...
			return ctx.getEditorUI().getColoring(path.getFullTokenName(JavaLayerTokenContext.METHOD));
		}
		...
		private boolean isMethod(DrawContext ctx) {
			...
			if (idEndOffset > resolvedEndOffset) { // beyond the resolved area
				...
				int nwOffset = Analyzer.findFirstNonWhite(buffer, idEndOffset - bufferStartOffset, endOffset - idEndOffset);
				if (nwOffset >= 0) { // found non-white
					...
					try {
						resolvedValue = (ctx.getEditorUI().getDocument().find(nwFinder, endOffset, -1) >= 0) && (nwFinder.getFoundChar() == '(');
