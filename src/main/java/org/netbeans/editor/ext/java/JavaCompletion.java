...
public class JavaCompletion extends Completion {
	...
	public static JCClass createSimpleClass(String fullClassName) {
		...
		return createSimpleClass(fullClassName.substring(nameInd), (nameInd > 0) ? fullClassName.substring(0, nameInd - 1) : ""); // NOI18N
	}
	...
	public static class SimpleClass implements JCClass {
		...
		public String toString() {
			return (getPackageName().length() > 0) ? getPackageName() + '.' + getName().replace('.', '$') : getName().replace('.', '$');
		}
