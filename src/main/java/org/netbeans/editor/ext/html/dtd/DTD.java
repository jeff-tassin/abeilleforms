...
/**
 * The interface representing SGMLish Document Type Definition. There is
...
public interface DTD {
	 ...
	 * 
	 * @return the name under which should be this DTD registered in DTD
	 *         registry.
	 */
	 ...
	 * 
	 * @param prefix
	 *            the prefix all returned Elements must start with. For empty
	 *            string or <CODE>null</CODE>, List of all Elements from this
	 *            DTD will be returned. The implementation <B>must</B> handle
	 *            <CODE>null</CODE> correctly.
	 ...
	 * 
	 * @return DTD.Element for given name or <CODE>null</CODE>, if no such
	 *         Element exists in this DTD.
	 */
	 ...
	 * 
	 * @param prefix
	 *            the requred prefix of CharRefs. For empty string or <CODE>null</CODE>,
	 *            List of all CharRefs from this DTD is returned. The
	 *            implementation <B>must</B> handle <CODE>null</CODE>
	 *            correctly.
	 ...
	 * 
	 * @return DTD.CharRef for given name or <CODE>null</CODE>, if no such
	 *         CharRef exists in this DTD.
	 */
	public DTD.CharRef getCharRef(String name);

	/**
	 * Element is the interface providing informations about HTML Element and
	 * its content model.
	 */
	public static interface Element {
		 ...
		 * 
		 * @param prefix
		 *            the requred prefix of Attributes. For empty string or
		 *            <CODE>null</CODE>, List of all Attributes of this
		 *            Element is returned. The implementation <B>must</B>
		 *            handle <CODE>null</CODE> correctly.
		 ...
		 * 
		 * @return DTD.Attribute for given name or <CODE>null</CODE>, if no
		 *         such Attribute exists in this Element.
		 */
	...
	/**
	 * Interface providing informations about one type of attribute. Every
	 * Element provides List of its' Attributes, which in turn provide.
	 * information about their types and possible values.
	 */
	public static interface Attribute {

		/**
		 * attribute of boolean type - the one which can't have "= smgt." after
		 * it
		 */
		...
		/**
		 * @return type of this attribute, could be TYPE_BOOLEAN, TYPE_SET or
		 *         TYPE_BASE
		 */
		public int getType();

		/**
		 * The base type of this attribute. Used only for TYPE_BASE attributes.
		 * 
		 * @return the base type, like CDATA, NUMBER, ID, if known (getType() ==
		 *         TYPE_BASE, null elsewhere.
		 */
		public String getBaseType();

		/**
		 * Only helper method, should return the last entity name through which
		 * was this Attribute's type defined. e.g. for color attrib in: <!ENTITY %
		 * Color "CDATA"> <ATTLIST FONT color %Color #IMPLIED> should this
		 * method return "Color". May return <CODE>null</CODE>.
		 */
		 ...
		 * 
		 * @returns the default value or one of MODE_IMPLIED, MODE_REQUIRED or
		 *          MODE_FIXED constants.
		 */
	...
	/**
	 * The interface representing Character reference. Provides its name and
	...
	/**
	 * The interface representing Content model of an Element. Content model is
	 * based on expression matching some sequence of Elements (the Content) and
	 * Set of added and excluded Elements. The point of added and excluded
	 * Elements is that they are "sticky" - are propagated down the hierarchy of
	 * Elements.
	 */
	public static interface ContentModel {
		 ...
		 *         ContentModel of all Elements inside the Element with this
		 *         ContentModel. Exclusion overrieds inclusion, but not
		 *         otherwise.
		 */
	...
	public static interface Content {
		static class EmptyContent implements Content {
			...
			public Content reduce(String name) {
				return null;
			}
		...
		/*
		 * public static Content EMPTY_CONTENT = new Content() { public boolean
		 * isDiscardable() { return true; } public Content reduce( String name ) {
		 * return null; } public Set getPossibleElements() { return new
		 * TreeSet(); } };
		 */
		/**
		 * Tells whether this content can be discarded - i.e. matches empty
		...
		/**
		 * Make a left reduction of this Content. Match the given element and
		 * create a content model of the rest. Notify caller, when given element
		 * doesn't match this Content
		 * 
		 * @return reduced Content, if element left-reduces the content, in the
		 *         case the reduction lead to empty content, return <CODE>EMPTY_CONTENT</CODE>.
	...
	public static interface ContentNode extends Content {
		...
		/**
		 * Get the operator for this node, could be unary ('+', '*', '?') or
		 * n-ary ('|', '&', ',')
		 */
		public char getType();
	}
