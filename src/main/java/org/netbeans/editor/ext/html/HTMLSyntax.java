...
public class HTMLSyntax extends Syntax {
	...
	private static final int ISI_ARG = 11; // Inside tag's argument - "<A
											// h_r_...>"
	private static final int ISP_ARG_X = 12; // X-switch after ARGUMENT's
												// name
	...
	private static final int ISA_REF = 28; // when comes to character
											// reference, e.g. &amp;, after &
	 ...
	 * to specs, following characters are treated as whitespace: Space - <CODE>'\u0020'</CODE>,
	 * Tab - <CODE>'\u0009'</CODE>, Formfeed - <CODE>'\u000C'</CODE>,Zero-width
	 * space - <CODE>'\u200B'</CODE>, Carriage return - <CODE>' '</CODE>
	 * and Line feed - <CODE>' '</CODE> CR's are included for completenes
	 * only, they should never appear in document
	 */
	...
	protected TokenID parseToken() {
		...
		while (offset < stopOffset) {
			...
			case ISA_REF_X:
				if ((actChar >= '0' && actChar <= '9') || (actChar >= 'a' && actChar <= 'f') || (actChar >= 'A' && actChar <= 'F')) {
					state = ISI_REF_HEX;
					break;
				}
				state = subState;
