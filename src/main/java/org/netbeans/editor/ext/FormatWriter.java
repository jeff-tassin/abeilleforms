...
/**
 * Formatting writter accepts the input-text, formats it and writes the output
 ...
 * format-layers registered in <tt>ExtFormatter</tt> and asking them for
...
public final class FormatWriter extends Writer {
	...
	public static final boolean debug = Boolean.getBoolean("netbeans.debug.editor.format"); // NOI18N

	/** Whether debug messages should be displayed */
	public static final boolean debugModify = Boolean.getBoolean("netbeans.debug.editor.format.modify"); // NOI18N
	...
	/**
	 * Syntax scanning the characters passed to the writer. For
	 * non-BaseDocuments it also scans the characters preceding the format
	 * offset. The goal here is to maintain the formatted tokens consistent with
	 * the scanning context at the offset in the document. This is achieved
	 * differently depending on whether the document is an instance of the
	 * BaseDocument or not. If the document is an instance of the
	 * <tt>BaseDocument</tt>, the syntax is used solely for the scanning of
	 * the formatted tokens and it is first prepared to scan right after the
	 * offset. For non-BaseDocuments the syntax first scans the whole are from
	 * the begining of the document till the offset and then continues on with
	 * the formatted tokens.
	 */
	private Syntax syntax;

	/**
	 * Whether the purpose is to find an indentation instead of formatting the
	 * tokens. In this mode only the '\n' is written to the format-writer, and
	 * the layers should insert the appropriate white-space tokens before the
	...
	/**
	 * Whether the first flush() is being done. it must respect the
	 * offsetPreScan.
	 */
	...
	/**
	 * This flag is set automatically if the new removal or insertion into chain
	 * occurs. The formatter can use this flag to detect whether a particular
	 * format-layer changed the chain.
	 */
	...
	/**
	 * Flag that helps to avoid unnecessary formatting when calling flush()
	 * periodically without calling write()
	 */
	private boolean lastFlush;

	/**
	 * Shift resulting indentation position to which the caret is moved. By
	 * default the caret goes to the first non-whitespace character on the
	 * formatted line. If the line is empty then to the end of the indentation
	 * whitespace. This variable enables to move the resulting position either
	 * left or right.
	 */
	private int indentShift;

	/**
	 * Whether this format writer runs in the simple mode. In simple mode the
	 * input is directly written to output.
	 */
	...
	/**
	 * The format writers should not be extended to enable operating of the
	...
	FormatWriter(ExtFormatter formatter, Document doc, int offset, Writer underWriter, boolean indentOnly) {
		...
		if (debug) {
			System.err.println("FormatWriter() created, formatter=" + formatter // NOI18N
					...
					? ((BaseDocument) doc).getTabSize()
							: formatter.getTabSize()) + ", shiftWidth=" + ((doc instanceof BaseDocument) // NOI18N
		...
		Class kitClass = (doc instanceof BaseDocument) ? ((BaseDocument) doc).getKitClass() : formatter.getKitClass();
		...
		if (doc instanceof BaseDocument) {
			try {
				...
				/*
				 * Init syntax right at the formatting offset so it will contain
				 * the prescan characters only. The non-last-buffer is inforced
				 * (even when at the document end) because the text will follow
				...
				if (offset > 0) { // only if not formatting from the start of
									// the document
					...
					int startOffset = Utilities.getRowStart(bdoc, Math.max(offset - offsetPreScan, 0), -Math.max(lines.intValue(), 1));
					...
					if (ti != null && ti.getOffset() < offset - offsetPreScan) {
						...
						while (lastToken.getNext() != null && lastToken.getNext().getOffset() < offset - offsetPreScan) {
			...
			try {
				...
				while (tokenID != null) {
					...
					lastToken = new FormatTokenItem(tokenID, syntax.getTokenContextPath(), tokenOffset, text.substring(tokenOffset, tokenOffset
							+ syntax.getTokenLength()), lastToken);
		...
		if (debug) {
			System.err.println("FormatWriter: writing preScan chars='" // NOI18N
					+ EditorDebug.debugChars(buf, bufOffset - offsetPreScan, offsetPreScan) + "'" // NOI18N
	...
	/**
	 * Get the first token that should be formatted. This can change as the
	 ...
	 * 
	 * @return the first token that should be formatted. It can be null in case
	 *         some layer removes all the tokens that should be formatted. Most
	 *         of the layers probably do nothing in case this value is null.
	 */
	...
	/**
	 * Get the first position that doesn't belong to the document. Initially
	 * it's the same as the <tt>getFormatStartPosition()</tt> but if there are
	...
	/**
	 * Get the last token in the chain. It can be null if there are no tokens in
	 * the chain.
	 */
	 ...
	 * 
	 * @param token
	 *            token from which the search for previous tokens will start. It
	 *            can be null in which case the last document token or last
	 *            token are attempted instead.
	 */
	public TokenItem findFirstToken(TokenItem token) {
		if (token == null) {
			// Try textStartPosition first
			token = (textStartPosition != null) ? textStartPosition.getToken() : null;
	...
	/**
	 * It checks whether the tested token is after some other token in the
	 * chain.
	 * 
	 * @param testedToken
	 *            token to test (whether it's after afterToken or not)
	 * @param afterToken
	 *            token to be compared to the testedToken
	 * @return true whether the testedToken is after afterToken or not. Returns
	 *         false if the token == afterToken or not or if token is before the
	 *         afterToken or not.
	 */
	...
	/**
	 * Check whether the given token has empty text and if so start searching
	 ...
	 * 
	 * @param token
	 *            token to start to search from. If it has zero length, the
	 *            search for non-empty token is performed in the given
	 *            direction.
	 */
	...
	/**
	 * Check whether a new token can be inserted into the chain before the given
	 ...
	 * 
	 * @param beforeToken
	 *            token-item before which the new token-item is about to be
	 *            inserted. It can be null to append the new token to the end of
	 *            the chain.
	 */
	...
	/**
	 * Create a new token-item and insert it before the token-item given as
	 * parameter. The <tt>canInsertToken()</tt> should be called first to
	 ...
	 *            the chain.
	 * @param tokenID
	 *            token-id of the new token-item
	 * @param tokenContextPath
	 *            token-context-path of the new token-item
	 * @param tokenImage
	 *            image of the new token-item
	 */
	...
	/**
	 * Whether the token-item can be removed. It can be removed only in case it
	...
	/**
	 * Remove the token-item from the chain. It can be removed only in case it
	 * doesn't come from the document's text and it wasn't yet written to the
	 * underlying writer.
	 */
	...
	/**
	 * Create the additional token from the text at the start of the given
	 * token.
	 * 
	 * @param token
	 *            token being split.
	 * @param startLength
	 *            length of the text at the begining of the token for which the
	 *            additional token will be created.
	 * @param tokenID
	 *            token-id that will be assigned to the new token
	 * @param tokenContextPath
	...
	public TokenItem splitStart(TokenItem token, int startLength, TokenID newTokenID, TokenContextPath newTokenContextPath) {
		...
		ExtTokenItem newToken = (ExtTokenItem) insertToken(token, newTokenID, newTokenContextPath, newText);
	 ...
	 *            token being split.
	 * @param endLength
	 *            length of the text at the end of the token for which the
	 *            additional token will be created.
	 * @param tokenID
	 *            token-id that will be assigned to the new token
	 * @param tokenContextPath
	...
	public TokenItem splitEnd(TokenItem token, int endLength, TokenID newTokenID, TokenContextPath newTokenContextPath) {
		...
		ExtTokenItem newToken = (ExtTokenItem) insertToken(token.getNext(), newTokenID, newTokenContextPath, newText);
	...
	/**
	 * Whether the token can be modified either by insertion or removal at the
	 * given offset.
	 */
	...
	/**
	 * Insert the text at the offset inside the given token. All the
	 * token-positions at and after the offset will be increased by
	 * <tt>text.length()</tt>. <tt>IllegalArgumentException</tt> is thrown
	 * if offset is wrong.
	 * 
	 * @param token
	 *            token in which the text is inserted.
	 * @param offset
	 *            offset at which the text will be inserted.
	 * @param text
	 *            text that will be inserted at the offset.
	 */
	...
	/**
	 * Remove the length of the characters at the given offset inside the given
	 * token. <tt>IllegalArgumentException</tt> is thrown if offset or length
	 * are wrong.
	 * 
	 * @param token
	 *            token in which the text is removed.
	 * @param offset
	 *            offset at which the text will be removed.
	 * @param length
	...
	public void remove(TokenItem token, int offset, int length) {
		...
		if (debugModify) {
			String removedText;
			if (offset >= 0 && length >= 0 && offset + length <= token.getImage().length()) {
	...
	/**
	 * Get the token-position that corresponds to the given offset inside the
	 ...
	 * 
	 * @param token
	 *            token in which the position is created.
	 * @param offset
	 *            inside the token at which the position will be created.
	 * @param bias
	 *            forward or backward bias
	 */
	...
	public boolean isChainStartPosition(FormatTokenPosition pos) {
		...
		return (pos.getOffset() == 0) && ((token == null && getLastToken() == null) // no
																					// tokens
	...
	public synchronized void write(char[] cbuf, int off, int len, int[] saveOffsets, Position.Bias[] saveBiases) throws IOException {
		...
		lastFlush = false; // signal write() was the last so flush() can be
							// done

		if (debug) {
			System.err.println("FormatWriter.write(): '" // NOI18N
					+ org.netbeans.editor.EditorDebug.debugChars(cbuf, off, len) + "', length=" + len + ", bufferSize=" + bufferSize); // NOI18N
		}
	...
	public void flush() throws IOException {
		...
		if (firstFlush) { // doing first flush
			if (startOffset > 0) { // check whether there's a preScan
				while (true) {
					String text = new String(buffer, syntax.getTokenOffset(), syntax.getTokenLength());
					// add a new token-item to the chain
					lastToken = new FormatTokenItem(tokenID, syntax.getTokenContextPath(), -1, text, lastToken);
					...
					if (text.length() > startOffset) {
						formatStartPosition = getPosition(lastToken, startOffset, Position.Bias.Backward);
					}

					tokenID = syntax.nextToken(); // get next token

					// When force last buffer is true, the XML token chain can
					// be split
					// into more than one token. This does not happen for Java
					// tokens.
					// Because of this split must all tokens which are part of
					// preScan
					// (means which have end position smaller than startOffset),
					// be changed to
		...
		while (tokenID != null) {
			...
			if (formatStartPosition == null) {
				formatStartPosition = getPosition(lastToken, 0, Position.Bias.Backward);
			}
		...
		// Write the output tokens to the underlying writer marking them as
		// written
		...
		if (sb.length() > 0) {
			...
			if (debug) {
				System.err.println("FormatWriter.flush(): chars to underlying writer='" // NOI18N
						+ EditorDebug.debugChars(outBuf, 0, outBuf.length) + "'"); // NOI18N
			}
	...
	private void checkFSPFollowsTSP() {
		if (!(formatStartPosition.equals(textStartPosition) || isAfter(formatStartPosition, textStartPosition))) {
			throw new IllegalStateException("formatStartPosition doesn't follow textStartPosition"); // NOI18N
		}
	}
	 ...
	 * 
	 * @param token
	 *            mark this token as current one. It can be null.
	 * @param maxDocumentTokens
	...
	/**
	 * Token-item created for the tokens that come from the text written to the
	 * writer.
	 */
	static class FormatTokenItem extends TokenItem.AbstractItem implements ExtTokenItem {

		/**
		 * How big part of the token was already written to the underlying
		 * writer. -1 means nothing was written yet, Integer.MAX_VALUE means
		 * everything was written and the token cannot be extended and some
		 * other value means how big part was already written.
		 */
		int writtenLength = -1;
		...
		/**
		 * Save offset used to give the relative position to the start of the
		 * current formatting. It's used by the FormatTokenPositionSupport.
		 */
		int saveOffset;
		...
		public void setWrittenLength(int writtenLength) {
			if (writtenLength <= this.writtenLength) {
				throw new IllegalArgumentException("this.writtenLength=" + this.writtenLength // NOI18N
		...
		FormatTokenItem insertToken(TokenID tokenID, TokenContextPath tokenContextPath, int offset, String image) {
			FormatTokenItem fti = new FormatTokenItem(tokenID, tokenContextPath, offset, image, previous);
			fti.next = this;
			this.previous = fti;
			return fti;
		}
	...
	/**
	 * This token item wraps every token-item that comes from the
	 * syntax-support.
	 */
	static class FilterDocumentItem extends TokenItem.FilterItem implements ExtTokenItem {
	...
	interface ExtTokenItem extends TokenItem {
		...
		/**
		 * Was this item already flushed to the underlying writer or does it
		 ...
		 * when determining whether there can be insert of the text made at the
		 * particular offset.
		 * 
		 * @return -1 to signal that token wasn't written at all. Or
		 *         Integer.MAX_VALUE to signal that the token was written and
		 *         cannot be extended. Or something else that means how long
		 *         part of the token was already written to the file.
		 */
		public int getWrittenLength();
