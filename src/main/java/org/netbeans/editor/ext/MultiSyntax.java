...
public class MultiSyntax extends Syntax {
	...
	/**
	 * Slave syntaxes that can be used for scanning. They can be added by
	 * registerSyntax().
	 */
	 ...
	 * other syntaxes. The state infos stored are identified by the their syntax
	 * classes.
	 */
	public static class MultiStateInfo extends BaseStateInfo {
		...
		/**
		 * Goes through all the syntaxes and inits them. If the multi-state-info
		 * has valid state-info for the given syntax the state-info is used.
			...
			while (syntaxItem != null) {
				if (syntaxItem.active) {
					...
					if (item == null) { // not found, add new
						item = stateInfoChain = new ChainItem(s.createStateInfo(), sc, stateInfoChain);
					}
		...
		static class ChainItem {
			...
			/**
			 * Whether this item is valid. It can become invalid if the syntax
			 * that it represents becomes inactive in this item.
			 */
			boolean valid;
			...
			/*
			 * Delta of the offset variable of the slave syntax against the
			 * offset variable of the master syntax.
			 */
	...
	static class SyntaxInfo {
		...
		/**
		 * Whether this syntax is actively scanning the text. There can be
		 * possibly more syntaxes scanning the in a nested way.
		 */
		boolean active;
