...
/**
 * Code completion querying support.
 * 
...
public interface CompletionQuery {
	...
	/**
	 * Result of the query or expression evaluation. Simply said it consists of
	 * the list of the data and title and an internal information about how to
	 * substitute the text.
	 */
	public interface Result {

		/**
		 * Get the list with the items satisfying the query. The list must
		 * always be non-null. If there are no data it will have a zero size.
		 * 
		...
		/**
		 * Substitute the text in the document if the user picks the item from
		 * the data with the given index either by pressing ENTER or
		 * doubleclicking the item by mouse.
		 * 
		 * @param dataIndex
		 *            current selected item index in the current data list. It
		 *            can be used for making the substitution.
		 * @param shift
		...
		/**
		 * Substitute the text that is common for all the data entries. This is
		 * used to update the document with the common text when the user
		 * presses the TAB key.
		 * 
		 * @param dataIndex
		 *            current selected item index in the current data list.
		 *            Although normally it shouldn't be necessary for making the
		 *            substitution, the completion implementations can use it
	...
	/**
	 * The very basic funztionality of Result is implemented by this class, but
	 * parts general enough to not need to be overriden.
	 */
	...
	public static class DefaultResult extends AbstractResult {
		 ...
		 * 
		 * @param component
		 *            the JTextComponent the result is tightened with, used for
		 *            operations on its Document, caret, selection and so.
		 * @param title
		 *            the title displayed in header of completion window
		 * @param data
		 *            the list of ResultItem instances to be displayed in
		 *            completion window, may be null.
		 * @param the
		 *            offset in the document corresponding to the start of the
		 *            text occassionally replaced by the result.
		 * @param the
		...
		/**
		 * Internal method used to find longest common prefix of two Strings. it
		 * is made private, because I'm going to change its interface for better
		 * performance.
		 */
		...
		/**
		 * Update the text in response to pressing TAB key. Searches through all
		 * items of this result looking for longest common prefix and then calls
		 * the substitution method on selected item providing it with the length
		 * of common part.
		 * 
	...
	/**
	 * An interface used as an item of List returned by
	 * CompletionQuery.Result.getData() Such items are then able to their part
	 * in Completion process themselves
	 */
	public static interface ResultItem {
		 ...
		 * 
		 * @param c
		 *            the text component to operate on, enables implementation
		 *            to do things like movement of caret.
		 * @param offset
		 ...
		 *            differently - enables more kinds of invocation of
		 *            substituteText
		 * @return whether the text was successfully updated
		 */
		 ...
		 * 
		 * @return the substitution text, usable e.g. for finding common
		 *         text/its' length
		 */
		 ...
		 * @param isSelected
		 *            tells if this item is just selected, for using proper
		 *            color scheme.
		 * @param cellHasFocus
	...
	public abstract static class AbstractResultItem implements CompletionQuery.ResultItem {
		...
		/**
		 * Create new ResultItem for given text, should be used in subclass
		 * constructors
		 */
		...
		/**
		 * Generic implementation, behaves just as described in specification in
		 * substituteCommonText() - removes <CODE>len</CODE> characters at
		 * <CODE>offset</CODE> out of document and then inserts <CODE>subLen<CODE>
		 * characters from the <CODE>text</CODE>
		 */
		public boolean substituteCommonText(JTextComponent c, int offset, int len, int subLen) {
		...
		/**
		 * Generic implementation, behaves just as described in specification in
		 * substituteText() - removes <CODE>len</CODE> characters at <CODE>offset</CODE>
		 * out of document and then inserts whole <CODE>text</CODE>. Ignores
		 * <CODE>shift</CODE> argument.
		 */
		public boolean substituteText(JTextComponent c, int offset, int len, boolean shift) {
			BaseDocument doc = (BaseDocument) c.getDocument();
			try {
