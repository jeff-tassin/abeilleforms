...
/**
 * Unlike the formatter class, the ExtFormatter concentrates on providing a
 * support for the real formatting process. Each formatter (there's only one per
 * each kit) can contain one or more formatting layers. The <tt>FormatLayer</tt>
 * operates over the chain of the tokens provided by the <tt>FormatWriter</tt>.
 ...
 * <tt>FormatWriter</tt> but the same set of format-layers is used for all the
 ...
 * redefined. The <tt>getSettingValue</tt> enables to get the up-to-date value
...
public class ExtFormatter extends Formatter implements FormatLayer {
	...
	/**
	 * Contains the names of the keys that were turned into custom settings and
	 * are no longer read from the Settings.
	 */
	...
	/**
	 * Return the name of this formatter. By default it's the name of the
	 * kit-class for which it's created without the package name.
	 */
	public String getName() {
		return getKitClass().getName().substring(getKitClass().getName().lastIndexOf('.') + 1);
	}
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		while (eit.hasNext()) {
			...
			if (settingName == null || e.getKey().equals(e.getKey())) {
				if (!customSettingsNamesMap.containsKey(e.getKey())) { // not
																		// custom
		...
		indentHotCharsAcceptor = SettingsUtil.getAcceptor(kitClass, ExtSettingsNames.INDENT_HOT_CHARS_ACCEPTOR, AcceptorFactory.FALSE);

		reindentWithTextBefore = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.REINDENT_WITH_TEXT_BEFORE, false);
	}
	...
	/**
	 * This method allows to set a custom value to a setting thus overriding the
	 * value retrieved from the <tt>Settings</tt>. Once done the value is no
	 * longer synchronized with the changes in <tt>Settings</tt> for the
	...
	/**
	 * Replace the format-layer with the layerName with the the given layer. If
	...
	/**
	 * Whether do no formatting at all. If this method returns true, the
	 ...
	 * @param endOffset
	 *            position at which the formatting ends
	 * @param indentOnly
	 *            whether just the indentation should be changed or regular
	 *            formatting should be performed.
	 * @return formatting writer. The text was already reformatted but the
	 *         writer can contain useful information.
	 */
	public Writer reformat(BaseDocument doc, int startOffset, int endOffset, boolean indentOnly) throws BadLocationException, IOException {
		...
		if (!fix5620 || fw == null) { // #5620 - for (fw != null) the doc was
										// already modified
	 ...
	 * 
	 * @param target
	 *            component to which the text was typed. Caaret position can be
	 *            checked etc.
	 * @param typedText
	 ...
	 *         reformatted. It can return block containing just one character.
	 *         The caller usually expands even one character to the whole line
	 *         because less than the whole line usually doesn't provide enough
	...
	public int[] getReformatBlock(JTextComponent target, String typedText) {
		...
		if (indentHotCharsAcceptor.accept(typedText.charAt(0))) {
			/*
			 * This is bugfix 10771. See the issue for problem description. The
			 * behaviour before fix was that whenever the lbrace is entered, the
			 * line is indented. This make no sense if a text exist on the line
			 * before the lbrace. In this case we simply will not indent the
			 * line. This is handled by the hasTextBefore check
			 */
		...
		if (doc instanceof BaseDocument) {
			...
			try {
				...
				if (eolOffset < 0) { // white line
					eolOffset = Utilities.getRowEnd(bdoc, offset);
				}
	...
	/**
	 * Whether the formatter accepts the given syntax that will be used for
	 * parsing the text passed to the FormatWriter.
	 * 
	 * @param syntax
	 *            syntax to be tested.
	 * @return true whether this formatter is able to process the tokens created
	 *         by the syntax or false otherwise.
	 */
