...
/**
 * Similair functionality as PropertyChangeSupport but holds only weak
 * references to listener classes
 * 
...
public class WeakPropertyChangeSupport {
	...
	/**
	 * Add weak listener to listen to change of the specified property. The
	 * caller must hold the listener object in some instance variable to prevent
	...
	public synchronized void removePropertyChangeListener(PropertyChangeListener l) {
		...
		for (int i = 0; i < cnt; i++) {
			...
			if (o == null || o == l) { // remove null references and the
										// required one
				listeners.remove(i);
