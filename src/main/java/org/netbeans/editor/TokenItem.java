...
public interface TokenItem {
	...
	/**
	 * Get next token-item in the text. It returns null if there's no more next
	 * tokens in the text. It can throw <tt>IllegalStateException</tt> in case
	 * the document was changed so the token-item chain becomes invalid.
	 */
	public TokenItem getNext();

	/**
	 * Get previous token-item in the text. It returns null if there's no more
	 * previous tokens in the text. It can throw <tt>IllegalStateException</tt>
	...
	public static abstract class AbstractItem implements TokenItem {
		...
		public String toString() {
			return "'" + org.netbeans.editor.EditorDebug.debugString(getImage()) + "', tokenID=" + getTokenID() + ", tcp=" + getTokenContextPath()
					+ ", offset=" + getOffset();
		}
