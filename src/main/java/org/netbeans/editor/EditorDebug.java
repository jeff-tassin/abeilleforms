...
public class EditorDebug {
	...
	public static void dumpPlanes(BaseDocument doc) {
		Class markClasses[] = new Class[] { MarkFactory.LineMark.class, MarkFactory.CaretMark.class, MarkFactory.DrawMark.class, MarkFactory.SyntaxMark.class,
				Mark.class };
		char markChars[] = new char[] { 'L', 'C', 'D', 'S', 'B' };
	...
	public static void dumpSyntaxMarks(final BaseDocument doc) {
		...
		/*
		 * doc.op.renderMarks( new DocMarks.Renderer() { public void render() {
		 * int markCnt = getMarkCnt(); int index = 0; int pos = 0; int lastPos =
		 * pos; int lastMarkPos = 0; int maxMarkDistance = 0; int
		 * minMarkDistance = docLen; Mark markArray[] = getMarkArray();
		 * SyntaxSeg.Slot slot = SyntaxSeg.getFreeSlot(); Syntax syntax =
		 * doc.getFreeSyntax();
		 * 
		 * try { syntax.load(null, slot.array, 0, 0, false, 0); while (index <
		 * markCnt) { Mark mark = markArray[index++]; pos += getRelPos(mark); if
		 * (mark instanceof MarkFactory.SyntaxMark) { MarkFactory.SyntaxMark
		 * syntaxMark = (MarkFactory.SyntaxMark)mark; int delta = pos -
		 * lastMarkPos; if (delta > maxMarkDistance) { maxMarkDistance = delta; }
		 * if (delta < minMarkDistance) { minMarkDistance = delta; } lastMarkPos =
		 * pos;
		 * 
		 * int preScan = syntax.getPreScan(); int loadPos = lastPos - preScan;
		 * int scanLen = pos - loadPos; try { slot.load(doc, loadPos, scanLen); }
		 * catch (BadLocationException e) { e.printStackTrace(); }
		 * syntax.relocate(slot.array, slot.offset + preScan, scanLen - preScan,
		 * (pos == docLen), pos);
		 * 
		 * while (syntax.nextToken() != null) { }
		 * 
		 * lastPos = pos; try { System.out.println(((syntaxMark ==
		 * doc.op.eolMark) ? "!!EOLMark!!" : "syntaxMark:") // NOI18N + "
		 * getOffset()=" + Utilities.debugPosition(doc, syntaxMark.getOffset()) //
		 * NOI18N + ", getLine()=" + syntaxMark.getLine() // NOI18N + ", " +
		 * syntaxMark // NOI18N + ",\n StateInfo=" + syntaxMark.getStateInfo() //
		 * NOI18N + ",\n Syntax:" + syntax); // NOI18N } catch
		 * (InvalidMarkException e) { e.printStackTrace(); } } } } finally {
		 * doc.releaseSyntax(syntax); SyntaxSeg.releaseSlot(slot); }
		 * 
		 * System.out.println("Maximum mark distance is " + maxMarkDistance //
	...
	public static void checkSettings(Class kitClass) throws Exception {
		int readBufferSize = SettingsUtil.getInteger(kitClass, SettingsNames.READ_BUFFER_SIZE, SettingsDefaults.defaultReadBufferSize);

		int writeBufferSize = SettingsUtil.getInteger(kitClass, SettingsNames.WRITE_BUFFER_SIZE, SettingsDefaults.defaultWriteBufferSize);

		int readMarkDistance = SettingsUtil.getInteger(kitClass, SettingsNames.READ_MARK_DISTANCE, SettingsDefaults.defaultReadMarkDistance);

		int markDistance = SettingsUtil.getInteger(kitClass, SettingsNames.MARK_DISTANCE, SettingsDefaults.defaultMarkDistance);

		int maxMarkDistance = SettingsUtil.getInteger(kitClass, SettingsNames.MAX_MARK_DISTANCE, SettingsDefaults.defaultMaxMarkDistance);

		int minMarkDistance = SettingsUtil.getInteger(kitClass, SettingsNames.MIN_MARK_DISTANCE, SettingsDefaults.defaultMinMarkDistance);

		int syntaxUpdateBatchSize = SettingsUtil.getInteger(kitClass, SettingsNames.SYNTAX_UPDATE_BATCH_SIZE, SettingsDefaults.defaultSyntaxUpdateBatchSize);
	...
	public static String debugString(String s) {
		return (s != null) ? debugChars(s.toCharArray(), 0, s.length()) : "NULL STRING";
	}
