...
public class Formatter implements SettingsChangeListener {
	 ...
	 * 
	 * @param kitClass
	 *            class of the kit for which the formatter is being assigned.
	 * @param formatter
	 *            new formatter for the given kit
	 */
	...
	private static final String[][] indentStringCache = new String[ISC_MAX_TAB_SIZE][];
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (!inited || settingName == null || SettingsNames.TAB_SIZE.equals(settingName)) {
			if (!customTabSize) {
				tabSize = SettingsUtil.getInteger(kitClass, SettingsNames.TAB_SIZE, SettingsDefaults.defaultTabSize);
			}
		}
		...
		if (!inited || settingName == null || SettingsNames.EXPAND_TABS.equals(settingName)) {
			if (!customExpandTabs) {
				expandTabs = SettingsUtil.getBoolean(kitClass, SettingsNames.EXPAND_TABS, SettingsDefaults.defaultExpandTabs);
			}
		}
		if (!inited || settingName == null || SettingsNames.SPACES_PER_TAB.equals(settingName)) {
			if (!customSpacesPerTab) {
				spacesPerTab = SettingsUtil.getInteger(kitClass, SettingsNames.SPACES_PER_TAB, SettingsDefaults.defaultSpacesPerTab);
			}
		}
	...
	/**
	 * Get the number of spaces the TAB character ('\t') visually represents for
	 ...
	 * <code>BaseDocument.getTabSize()</code> must be used for BaseDocuments
	...
	/**
	 * Set the number of spaces the TAB character ('\t') visually represents for
	...
	/**
	 * Set the width of one indentation level for non-BaseDocument documents. It
	...
	static String getIndentString(int indent, boolean expandTabs, int tabSize) {
		...
		synchronized (Settings.class) {
			boolean large = (tabSize >= indentStringCache.length) || (indent > ISC_MAX_INDENT_SIZE); // indexed
																										// by
																										// (indent
																										// - 1)
	...
	/**
	 * Get the string that is appropriate for the requested indentation. The
	 * returned string respects the <tt>expandTabs()</tt> and the
	 * <tt>getTabSize()</tt> and will contain either spaces only or fully or
	...
	/**
	 * Modify the line to move the text starting at dotPos one tab column to the
	...
	/**
	 * Increase/decrease indentation of the block of the code. Document is
	...
	/**
	 * Creates a writer that formats text that is inserted into it. The writer
	 ...
	 *            document
	 * @param offset
	 *            position to begin inserts at
	 * @param writer
	 *            writer to write to
	 * @return new writer that will format written text and pass it into the
	 *         writer
	 */
