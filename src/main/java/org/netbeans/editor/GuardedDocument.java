...
public class GuardedDocument extends BaseDocument implements StyledDocument {
	...
	/**
	 * Guarded attribute used for specifying that the inserted block will be
	 * guarded.
	 */
	...
	/**
	 * Name of the normal style. The normal style is used to reset the effect of
	 ...
	 * 
	 * @param evt
	 *            document event containing the change including array of
	...
	protected void preInsertCheck(int offset, String text, AttributeSet a) throws BadLocationException {
		...
		if (text.length() > 0 && (rel & MarkBlock.OVERLAP) != 0 && rel != MarkBlock.INSIDE_END // guarded
																								// blocks
																								// have
																								// insertAfter
																								// endMark
	...
	public void setCharacterAttributes(int offset, int length, AttributeSet s, boolean replace) {
		if (((Boolean) s.getAttribute(GUARDED_ATTRIBUTE)).booleanValue() == true) {
			guardedBlockChain.addBlock(offset, offset + length, false); // no
																		// concat
	...
	public void setLogicalStyle(int pos, Style s) {
		try {
			...
			DrawLayerFactory.StyleLayer styleLayer = (DrawLayerFactory.StyleLayer) findLayer(layerName);
			if (styleLayer != null) {
		...
		fireChangedUpdate(createDocumentEvent(pos, 0, DocumentEvent.EventType.CHANGE)); // enough
																						// to
																						// say
																						// length
																						// 0
	...
	public String toStringDetail() {
		return super.toStringDetail() + getDrawLayerList() + ",\nGUARDED blocks:\n" + guardedBlockChain; // NOI18N
	}
