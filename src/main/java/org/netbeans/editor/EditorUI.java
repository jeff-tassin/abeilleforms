...
/**
 * Editor UI for the component. All the additional UI features like advanced
 * scrolling, info about fonts, abbreviations, keyword matching are based on
 * this class.
 * 
...
public class EditorUI implements ChangeListener, PropertyChangeListener, SettingsChangeListener {
	...
	/**
	 * Default scrolling type is used for the standard setDot() call. If the
	...
	/**
	 * Scrolling type used for regular caret moves. The scrollJump is used when
	 * the caret requests area outside the screen.
	 */
	public static final int SCROLL_MOVE = 1;

	/**
	 * Scrolling type where the smallest covering for the requested rectangle is
	 * used. It's useful for going to the end of the line for example.
	 */
	public static final int SCROLL_SMALLEST = 2;

	/**
	 * Scrolling type for find operations, that can request additional
	 ...
	 * minimize the amount of the coloring maps and also save the time necessary
	 * for their creation.
	 */
	...
	/**
	 * Enable displaying line numbers. Both this flag and
	 * <tt>lineNumberVisibleSetting</tt> must be true to have the line numbers
	...
	/**
	 * Whether to show line numbers or not. This flag is obtained using bitwise
	...
	/**
	 * Width of one digit used for line numbering. It's based on the information
	...
	/**
	 * This is the size of the editor as component while the real size of the
	...
	/**
	 * This is the full margin around the text. The left margin is an addition
	 * of component's margin and lineNumberWidth and textLeftMarginWidth.
	 */
	Insets textMargin = NULL_INSETS;

	/**
	 * How much columns/lines to add when the scroll is performed so that the
	...
	/**
	 * Flag saying whether either the width or height in virtualSize were
	 * updated.
	 */
	...
	/**
	 * The line numbers can be shown in glyph gutter and therefore it is
	 * necessary to disable drawing of lines here. During the printing on the
	 * the other hand, line numbers must be visible.
	 */
	...
	/**
	 * Gets the coloring map that can be shared by the components with the same
	...
	/**
	 * Get the lock assuring the component will not be changed by
	 * <tt>installUI()</tt> or <tt>uninstallUI()</tt>. It's useful for the
	 * classes that want to listen for the component change in <tt>EditorUI</tt>.
	 */
	...
	public void settingsChange(SettingsChangeEvent evt) {
		if (component != null) {
			if (Utilities.getKit(component) == null) {
				return; // prevent problems if not garbage collected and
						// settings changed
			}
		}
		...
		if (settingName == null || SettingsNames.LINE_NUMBER_VISIBLE.equals(settingName) || SettingsNames.PRINT_LINE_NUMBER_VISIBLE.equals(settingName)) {
			lineNumberVisibleSetting = SettingsUtil.getBoolean(kitClass, (component != null) ? SettingsNames.LINE_NUMBER_VISIBLE
					: SettingsNames.PRINT_LINE_NUMBER_VISIBLE, (component != null) ? SettingsDefaults.defaultLineNumberVisible
					: SettingsDefaults.defaultPrintLineNumberVisible);
			lineNumberVisible = lineNumberEnabled && lineNumberVisibleSetting;

			// if this is printing, the drawing of original line numbers must be
			// enabled
			if (component == null)
		...
		if (doc != null) {
			...
			if (settingName == null || SettingsNames.TEXT_LEFT_MARGIN_WIDTH.equals(settingName)) {
				textLeftMarginWidth = SettingsUtil.getInteger(kitClass, SettingsNames.TEXT_LEFT_MARGIN_WIDTH, SettingsDefaults.defaultTextLeftMarginWidth);
			}
			...
			if (settingName == null || SettingsNames.TEXT_LIMIT_LINE_VISIBLE.equals(settingName)) {
				textLimitLineVisible = SettingsUtil.getBoolean(kitClass, SettingsNames.TEXT_LIMIT_LINE_VISIBLE, SettingsDefaults.defaultTextLimitLineVisible);
			}
			...
			if (settingName == null || SettingsNames.TEXT_LIMIT_LINE_COLOR.equals(settingName)) {
				...
				textLimitLineColor = (value instanceof Color) ? (Color) value : SettingsDefaults.defaultTextLimitLineColor;
			}

			if (settingName == null || SettingsNames.TEXT_LIMIT_WIDTH.equals(settingName)) {
				textLimitWidth = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.TEXT_LIMIT_WIDTH, SettingsDefaults.defaultTextLimitWidth);
			}
			...
			if (component != null) {
				...
				if (settingName == null || SettingsNames.CARET_COLOR_INSERT_MODE.equals(settingName)
					...
					if (b == null || !b.booleanValue()) {
						...
						caretColor = (value instanceof Color) ? (Color) value : SettingsDefaults.defaultCaretColorInsertMode;

					}
					else {
						Object value = Settings.getValue(kitClass, SettingsNames.CARET_COLOR_OVERWRITE_MODE);
						caretColor = (value instanceof Color) ? (Color) value : SettingsDefaults.defaultCaretColorOvwerwriteMode;
					}
					...
					if (fm != null) {
						if (debugUpdateLineHeight) {
							if (maxHeight < fm.getHeight()) {
								System.err.println("Updating maxHeight from " + maxHeight + " to " + fm.getHeight() + ", coloringName=" + coloringName
										+ ", font=" + font);
							}

							if (maxHeight < fm.getHeight()) {
								System.err.println("Updating maxAscent from " + maxAscent + " to " + fm.getAscent() + ", coloringName=" + coloringName
										+ ", font=" + font);
							}
						}
	...
	protected void update(Graphics g) {
		...
		/*
		 * JDK1.3 patch for the behavior that occurs when the line is wider than
		 * the screen and the user first clicks End key to go to the end and
		 * then goes back by (Ctrl+)Left. As the non-simple scrolling mode is
		 * used in JViewport in 1.3 the line number block appears shifted to the
		 * right and gets repainted after 300ms which looks ugly. The patch is
		 * to set the simple scrolling mode into JViewport.
		 * 
		 * getParentViewport().setScrollMode(0); // 2 stands for
		 * SIMPLE_SCROLL_MODE
		 * 
		 */
		try {
	...
	/**
	 * Create or get extended editor component. The extended component should
	...
	/**
	 * Add new layer and use its priority to position it in the chain. If
	 * there's the layer with same visibility then the inserted layer will be
	 * placed after it.
	 * 
	 * @param layer
	 *            layer to insert into the chain
	 */
	...
	/**
	 * Possibly update virtual width. If the width is really updated, the method
	 * returns true.
	 */
	public boolean updateVirtualWidth(int width) {
		...
		if (width > virtualSize.width) {
			...
			widthInc = (widthInc < 0) ? (lastExtentBounds.width * (-widthInc) / 100) : widthInc * defaultSpaceWidth;
	...
	/**
	 * Possibly update virtual height. If the height is really updated, the
	...
	public boolean updateVirtualHeight(int height) {
		...
		if (height > 0) {
			if (height > virtualSize.height) {
				...
				heightInc = (heightInc < 0) ? (lastExtentBounds.height * (-heightInc) / 100) : heightInc * lineHeight;
		...
		else { // compute real height - fix of #14295
	 ...
	 * 
	 * @param maxDigitCount
	 *            maximum digit count that can the line number have. if it's
	 *            lower or equal to zero it will be computed automatically.
	 */
	...
	/**
	 * Scroll the view so that requested rectangle is best visible. There are
	 * different scroll policies available.
	 * 
	...
	private boolean scrollRectToVisibleImpl(Rectangle r, int scrollPolicy, Rectangle bounds) {
		...
		if (scrollPolicy == SCROLL_FIND) {
			// converted inset
			int cnvFI = (scrollFindInsets.left < 0) ? (-bounds.width * scrollFindInsets.left / 100) : scrollFindInsets.left * defaultSpaceWidth;

			int nx = Math.max(r.x - cnvFI, 0);

			cnvFI = (scrollFindInsets.right < 0) ? (-bounds.width * scrollFindInsets.right / 100) : scrollFindInsets.right * defaultSpaceWidth;

			r.width += (r.x - nx) + cnvFI;
			r.x = nx;

			cnvFI = (scrollFindInsets.top < 0) ? (-bounds.height * scrollFindInsets.top / 100) : scrollFindInsets.top * lineHeight;

			int ny = Math.max(r.y - cnvFI, 0);

			cnvFI = (scrollFindInsets.bottom < 0) ? (-bounds.height * scrollFindInsets.bottom / 100) : scrollFindInsets.bottom * lineHeight;
		...
		if (r.width > bounds.width || r.height > bounds.height) {
			...
			if (caretRect.x >= r.x && caretRect.x + caretRect.width <= r.x + r.width && caretRect.y >= r.y && caretRect.y + caretRect.height <= r.y + r.height) { // caret
																																									// inside
																																									// requested
																																									// rect
		...
		if (r.x < bounds.x) {
			...
			switch (scrollPolicy) {
			case SCROLL_MOVE:
				newX = (scrollJumpInsets.left < 0) ? (bounds.width * (-scrollJumpInsets.left) / 100) : scrollJumpInsets.left * defaultSpaceWidth;
			...
			case SCROLL_SMALLEST:
				...
				break;
			default:
				newX = (scrollJumpInsets.right < 0) ? (bounds.width * (-scrollJumpInsets.right) / 100) : scrollJumpInsets.right * defaultSpaceWidth;
		...
		if (r.y < bounds.y) {
			...
			case SCROLL_MOVE:
				newY = r.y;
				newY -= (scrollJumpInsets.top < 0) ? (bounds.height * (-scrollJumpInsets.top) / 100) : scrollJumpInsets.top * lineHeight;
				break;
			case SCROLL_SMALLEST:
				newY = r.y;
				break;
			...
			case SCROLL_MOVE:
				...
				newY += (scrollJumpInsets.bottom < 0) ? (bounds.height * (-scrollJumpInsets.bottom) / 100) : scrollJumpInsets.bottom * lineHeight;
				break;
			case SCROLL_SMALLEST:
				newY = (r.y + r.height) - bounds.height;
				break;
	...
	public void adjustWindow(int caretPercentFromWindowTop) {
		...
		if (component != null && (component.getCaret() instanceof Rectangle)) {
			...
			bounds.y = caretRect.y - (caretPercentFromWindowTop * bounds.height) / 100 + (caretPercentFromWindowTop * lineHeight) / 100;
			Utilities.runInEventDispatchThread(new Runnable() {
				public void run() {
	...
	/**
	 * Set the dot according to the currently visible screen window. #param
	...
	public void adjustCaret(int percentFromWindowTop) {
		...
		if (c != null) {
			Rectangle bounds = getExtentBounds();
			bounds.y += (percentFromWindowTop * bounds.height) / 100 - (percentFromWindowTop * lineHeight) / 100;
			try {
	 ...
	 * 
	 * @param offset
	 *            position where the caret should be placed
	 * @param scrollRect
	 *            rectangle that should become visible. It can be null when no
	...
	/**
	 * This method is called by textui to do the paint. It is forwarded either
