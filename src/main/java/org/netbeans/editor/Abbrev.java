...
/**
 * Abbreviation support allowing to expand defined character sequences into the
...
public class Abbrev implements SettingsChangeListener, PropertyChangeListener {
	...
	/**
	 * Abbreviation accounting string. Here the characters forming abbreviation
	 * are stored.
	 */
	 ...
	 * 
	 * @param abbrev
	 *            string to translate. Pass null to translate current
	 *            abbreviation string
		...
		if (abbrevStr != null && expansion != null && dotPos >= abbrevStrLen) {
			if (checkDocText) {
				try {
					...
					if (prevChars.equals(abbrevStr)) { // abbrev chars really
														// match text
	...
	/**
	 * Expand abbreviation on current caret position. Remove characters back to
	 * the word start and insert expanded abbreviation.
	 * 
	 * @return whether the typed character should be added to the abbreviation
	 *         or not
	 */
