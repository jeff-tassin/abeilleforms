...
/**
 * Various useful editor functions. Some of the methods have the same names and
...
public class Utilities {
	 ...
	 *            position in document where to start searching
	 * @param lineShift
	 *            shift the given offset forward/back relatively by some amount
	 *            of lines
	 * @return position of the start of the row or -1 for invalid position
	 */
	public static int getRowStart(BaseDocument doc, int offset, int lineShift) throws BadLocationException {
		if (lineShift != 0) {
			return doc.op.getBOLRelLine(offset, lineShift);
		}
		else { // no shift
	...
	/**
	 * Get indentation on the current line. If this line is white then go either
	 * up or down an return indentation of the first non-white row. The
	 * <tt>getRowFirstNonWhite()</tt> is used to find the indentation on
	 * particular line.
	 * 
	 * @param doc
	 *            document to operate on
	 ...
	 *            position in document anywhere on the line
	 * @param downDir
	 *            if this flag is set to true then if the row is white then the
	 *            indentation of the next first non-white row is returned. If
	 *            it's false then the indentation of the previous first
	 *            non-white row is returned.
	 * @return indentation or -1 if there's no non-white line in the specified
	 *         direction
	 */
	...
	/**
	 * Get the position that is one line above and visually at some x-coordinate
	 * value.
	 * 
	 * @param doc
	 *            document to operate on
	 ...
	 * @return position of the character that is at the one line above at the
	 *         required x-coordinate value
	 */
	...
	public static int getWordEnd(BaseDocument doc, int offset) throws BadLocationException {
		return doc.find(new FinderFactory.NextWordFwdFinder(doc, false, true), offset, -1);
	}
	 ...
	 *            position in document where to start searching
	 * @param limitPos
	 *            position in document (greater or equal than offset) where the
	 *            search will stop reporting unsuccessful search by returning -1
	...
	/**
	 * Get first white character in document in backward direction. The
	 ...
	 *            position in document where to start searching
	 * @param limitPos
	 *            position in document (lower or equal than offset) where the
	 *            search will stop reporting unsuccessful search by returning -1
	 * @return position of the first white character or -1
	 */
	 ...
	 *            document to operate on
	 * @param offset
	 *            position in document for which the visual column should be
	 *            found
	 ...
	 * @JMT custom method
	 */
	public static boolean isEOL(BaseDocument doc, int offset) throws BadLocationException {
		return doc.op.isEOL(offset);
	}
			...
			if (idEnd >= 0) {
				...
				if (id.trim().length() == 0) {
					...
					if (offset == javax.swing.text.Utilities.getWordEnd(c, prevWordStart))
						id = doc.getText(prevWordStart, offset - prevWordStart);
				}
	 ...
	 *            document to operate on
	 * @param offset
	 *            position in document determines the changed area begining
	 * @param len
	...
	public static boolean isRowWhite(BaseDocument doc, int offset) throws BadLocationException {
		offset = doc.find(new FinderFactory.NonWhiteFwdFinder(doc), doc.op.getBOL(offset), doc.op.getEOL(offset));
		return (offset == -1);
	}
	 ...
	 * @param startOffset
	 *            offset at which the formatting starts
	 * @param endOffset
	...
	/**
	 * Helper method to obtain instance of editor kit from existing
	 * JTextComponent. If the kit of the component is not an instance of the
	 * <tt>org.netbeans.editor.BaseKit</tt> the method returns null. The
	...
	/**
	 * Helper method to obtain instance of BaseDocument from JTextComponent. If
	 * the document of the component is not an instance of the
	 * <tt>org.netbeans.editor.BaseDocument</tt> the method returns null. The
	 * method doesn't require any document locking.
	 * 
	 * @param target
	 ...
	 * can be obtained by calling <tt>doc.getSyntaxSupport()</tt>. The method
	...
	/**
	 * Creates nice textual representation of KeyStroke. Modifiers and an actual
	...
	public static String keyStrokeToString(KeyStroke stroke) {
		...
		String keyText = (stroke.getKeyCode() == KeyEvent.VK_UNDEFINED) ? String.valueOf(stroke.getKeyChar()) : getKeyText(stroke.getKeyCode());
	...
	/**
	 * @return slight modification of what KeyEvent.getKeyText() returns. The
	 *         numpad Left, Right, Down, Up get extra result.
	 */
