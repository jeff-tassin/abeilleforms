...
/**
 * This is the parent of majority of the actions. It implements the necessary
...
public abstract class BaseAction extends TextAction {
	...
	/**
	 * This method is called once after the action is constructed and then each
	 * time the settings are changed.
	 * 
	 * @param evt
	 *            event describing the changed setting name. It's null if it's
	 *            called after the action construction.
	 * @param kitClass
	 *            class of the kit that created the actions
	 */
	protected void settingsChange(SettingsChangeEvent evt, Class kitClass) {
	}

	/**
	 * This method is made final here as there's an important processing that
	 ...
	 * 
	 * @param evt
	 *            action event describing the action that occured
	 * @param target
	 *            target component where the action occured. It's retrieved by
	 *            the TextAction.getTextComponent(evt).
	 */
	...
	/**
	 * Update the component according to the update mask specified in the
