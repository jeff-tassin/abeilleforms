...
class SyntaxSeg extends Segment {
	...
	/**
	 * From this position on, the data in syntax segment must be marked invalid.
	 */
	...
	static class Slot extends Segment {
		 ...
		 * 
		 * @return real length that was loaded (syntax segment has limitation in
		 *         size)
		 */
		int load(BaseDocument doc, int pos, int len) throws BadLocationException {
			if (len <= 0) {
				...
				throw new RuntimeException("len=" + len); // Critical error
															// NOI18N
			}
			...
			if (difDoc // different documents
					...
					|| pos - segPos + len > array.length) { // wouldn't fit
		...
		boolean isAreaInside(BaseDocument doc, int pos, int len) {
			return (doc == (BaseDocument) segDocRef.get() && pos >= segPos && pos + len <= segPos + segLen);
		}
		 ...
		 * 
		 * @param doc
		 *            document in which the change occured
		 * @param pos
		 *            position in the document where the change occured
		 */
