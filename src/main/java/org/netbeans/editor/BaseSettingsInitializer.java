...
/**
 * Initializer for the editor settings. This initializer contains the default
 * values for the majority of the base-level settings. The constants defined
 * here are used also internally as the default values for the editor in cases
 * the user enters an invalid value into some setting that would break the
...
public class BaseSettingsInitializer extends Settings.AbstractInitializer {
	...
	public void updateSettingsMap(Class kitClass, Map settingsMap) {
		...
		if (kitClass == BaseKit.class) {
			...
			// Base key-bindings
			SettingsUtil.updateListSetting(settingsMap, SettingsNames.KEY_BINDING_LIST, SettingsDefaults.defaultKeyBindings);

			// Base colorings
			SettingsUtil.setColoring(settingsMap, SettingsNames.DEFAULT_COLORING, SettingsDefaults.defaultColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.LINE_NUMBER_COLORING, SettingsDefaults.defaultLineNumberColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.BOOKMARK_COLORING, SettingsDefaults.defaultBookmarkColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.GUARDED_COLORING, SettingsDefaults.defaultGuardedColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.SELECTION_COLORING, SettingsDefaults.defaultSelectionColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.HIGHLIGHT_SEARCH_COLORING, SettingsDefaults.defaultHighlightSearchColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.INC_SEARCH_COLORING, SettingsDefaults.defaultIncSearchColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.STATUS_BAR_COLORING, SettingsDefaults.defaultStatusBarColoring);

			SettingsUtil.setColoring(settingsMap, SettingsNames.STATUS_BAR_BOLD_COLORING, SettingsDefaults.defaultStatusBarBoldColoring);

			// List of the colorings for all the kits
			SettingsUtil.updateListSetting(settingsMap, SettingsNames.COLORING_NAME_LIST, SettingsDefaults.defaultColoringNames);

			/*
			 * This was removed because document now uses this algorithm by
			 * default. Providing an explicit value for the setting means that
			 * the default algorithm should be overriden. Although the
			 * substituter below seems OK it doesn't reflect possible custom
			 * indent engines that can be provided by the IDE.
			 * 
			 * 
			 * settingsMap.put(SettingsNames.INDENT_SHIFT_WIDTH, // substitute
			 * new Settings.Evaluator() { public Object getValue(Class
			 * kitClass2, String settingName) { Object ret; if
			 * (SettingsUtil.getBoolean(kitClass2, SettingsNames.EXPAND_TABS,
			 * false)) { ret = Settings.getValue(kitClass2,
			 * SettingsNames.SPACES_PER_TAB); } else { // don't expand tabs ret =
			 * Settings.getValue(kitClass2, SettingsNames.TAB_SIZE); } return
			 * ret; } } );
			 */

			/*
			 * WARNING! The following settings should be changed with care as
			 * there are some dependencies among the values of these settings.
			 * If the values are wrong the editor may work in a wrong way in
			 * some circumstances. The EditorDebug.checkSettings(kitClass)
			 * should be run in some DOC_INSTALL_ACTION_LIST action to ensure
			 * that the changed values are correct.
			 */
