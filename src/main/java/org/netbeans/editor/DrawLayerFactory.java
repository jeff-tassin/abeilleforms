...
public class DrawLayerFactory {
	...
	/**
	 * Layer that colors the text according to the tokens that were parsed. It's
	 * active all the time.
	 */
	...
	/**
	 * This layer colors the line by a color specified in constructor It
	 * requires only activation mark since it deactivates automatically at the
	 * end of line.
	 */
	...
	/**
	 * Layer that covers selection services provided by caret. This layer
	 * assumes that both caretMark and selectionMark in BaseCaret are properly
	 * served so that their active flags are properly set.
	 */
	...
	/**
	 * Highlight search layer highlights all occurences of the searched string
	 * in text.
	 */
	public static class HighlightSearchLayer extends DrawLayer.AbstractLayer {
		...
		public void init(DrawContext ctx) {
			if (enabled) {
				try {
					BaseDocument doc = ctx.getEditorUI().getDocument();
					blocks = FindSupport.getFindSupport().getBlocks(blocks, doc, ctx.getStartOffset(), ctx.getEndOffset());
				} catch (BadLocationException e) {
					blocks = new int[] { -1, -1 };
				}
	...
	/**
	 * Layer covering incremental search. There are just two positions begining
	 * and end of the searched string
	 */
	...
	public static class WordColoringLayer extends DrawLayer.AbstractLayer {
		...
		public void updateContext(DrawContext ctx) {
			Coloring c = (Coloring) stringMap.get(ctx.getBuffer(), ctx.getTokenOffset(), ctx.getTokenLength());
			if (c != null) {
				c.apply(ctx);
			}
		}
	...
	/**
	 * Annotation layer for drawing of annotations. Each mark which is stored in
	 * markChain has corresponding Annotation. More than one Annotation can
	 * share one mark. In this case the only one annotation is active and this
	 * must be drawn.
	 */
