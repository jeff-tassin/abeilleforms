...
public class WordMatch extends FinderFactory.AbstractFinder implements SettingsChangeListener, PropertyChangeListener {
	...
	/**
	 * First part of matching word expressed as char[]. Status of word matching
	...
	/**
	 * Current index in word match vector. Reaching either first or last index
	...
	/**
	 * Whether the match should be reported when word is found which is only one
	 * char long.
	 */
	boolean matchOneChar;

	/**
	 * Maximum lenght in chars of the search area. If the number is zero, no
	...
	/**
	 * Called when settings were changed. The method is called by editorUI when
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (kitClass != null) {
			maxSearchLen = SettingsUtil.getInteger(kitClass, SettingsNames.WORD_MATCH_SEARCH_LEN, Integer.MAX_VALUE);
			wrapSearch = SettingsUtil.getBoolean(kitClass, SettingsNames.WORD_MATCH_WRAP_SEARCH, true);
			matchOneChar = SettingsUtil.getBoolean(kitClass, SettingsNames.WORD_MATCH_MATCH_ONE_CHAR, true);
			matchCase = SettingsUtil.getBoolean(kitClass, SettingsNames.WORD_MATCH_MATCH_CASE, false);
			smartCase = SettingsUtil.getBoolean(kitClass, SettingsNames.WORD_MATCH_SMART_CASE, false);
		}
	}
	 ...
	 * 
	 * @param forward
	 *            in which direction should the search be done
	 */
	public synchronized String getMatchWord(int startPos, boolean forward) {
		int listSize = wordInfoList.size();
		boolean searchNext = (listSize == 0) || (wordsIndex == (forward ? (listSize - 1) : 0));
		...
		if (baseWord == null) {
			try {
				...
				WordInfo info = new WordInfo(baseWordString, startDoc.createPosition(startPos - baseWord.length), startDoc);
				...
				while (doc != null) {
					if (doc.getLength() > 0) {
						...
						if (foundPos != -1) { // found
							...
							WordInfo info = new WordInfo(new String(word, 0, wordLen), doc.createPosition(foundPos), doc);
							wordsMap.put(info.word, info);
		...
		if (forwardSearch) {
			...
			while (offset < offset2) {
				...
				if (!wp) {
					...
					if (limitOffset == offset) {
						if (checkWord()) {
							found = true;
							// differs in one char because current is part of
							// word
			...
			while (offset >= offset1) {
				...
				if (!wp || (limitOffset == offset)) {
					if (wordLen > 0) {
						Analyzer.reverse(word, wordLen); // reverse word
															// chars
						if (checkWord()) {
							found = true;
	...
	private BaseDocument getStaticWordsDoc() {
		...
		if (doc == null) {
			String staticWords = (String) Settings.getValue(kitClass, SettingsNames.WORD_MATCH_STATIC_WORDS);
			if (staticWords != null) {
				doc = new BaseDocument(BaseKit.class, false); // don't add to
																// registry
				try {
	...
	/**
	 * Word match info - used in previous/next word matching. It contains info
	 * found word and next matching position.
	 */
	private static final class WordInfo {
		...
		/**
		 * Position of the word in document. Positions are used so that the
		 * marks are removed when they are no longer necessary.
		 */
		Position pos;
