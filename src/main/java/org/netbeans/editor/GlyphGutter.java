...
/**
 * GlyphGutter is component for displaying line numbers and annotation glyph
 * icons. Component also allow to "cycle" through the annotations. It means that
 * if there is more than one annotation on the line, only one of them might be
 * visible. And clicking the special cycling button in the gutter the user can
...
public class GlyphGutter extends JComponent implements Annotations.AnnotationsListener, Accessible {
	...
	/**
	 * Flag whther the gutter was initialized or not. The painting is disabled
	 * till the gutter is not initialized
	 */
	...
	/**
	 * The gutter height is enlarged by number of lines which specifies this
	 * constant
	 */
	private static final int ENLARGE_GUTTER_HEIGHT = 300;

	/**
	 * The hightest line number. This value is used for calculating width of the
	 * gutter
	 */
	...
	/*
	 * These two variables are used for caching of count of line annos on the
	 * line over which is the mouse caret. Just for sake of optimalization.
	 */
	...
	public GlyphGutter(EditorUI editorUI) {
		...
		// Annotations class is model for this view, so the listener on changes
		// in
	...
	/*
	 * Read accessible context @return - accessible context
	 */
	...
	protected void init() {
		...
		// TODO: clicking the line number abd dragging the mouse should select
		// block of lines
		...
		// after the glyph icons are loaded it is necessary to repaint the
		// gutter
		...
		AnnotationTypes.getTypes().addPropertyChangeListener(annoTypesListener = new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				if (evt.getPropertyName() == AnnotationTypes.PROP_GLYPHS_OVER_LINE_NUMBERS || evt.getPropertyName() == AnnotationTypes.PROP_SHOW_GLYPH_GUTTER) {
					update();
				}
			}
		});
	...
	/**
	 * Update colors, fonts, sizes and invalidate itself. This method is called
	 * from EditorUI.update()
	 */
	public void update() {
		...
		// fix for issue #16940
		// the real cause of this problem is that closed document is not garbage
		// collected,
		// because of *some* references (see #16072) and so any change in
		// AnnotationTypes.PROP_*
		// properties is fired which must update this component although it is
		// not visible anymore
	...
	public void paintComponent(Graphics g) {
		...
		// @JMT -- added to netbeans src. Draws a dark vertical line to make the
		// editor boundary more pronounced
		g.setColor(Color.black);
		int e_x = getWidth() - 1;
		g.drawLine(e_x, drawHere.y, e_x, drawHere.y + drawHere.height);
		...
		// "+(lineHeight/2)" means to don't draw less than half of the line
		// number
		while ((y + (lineHeight / 2)) <= (drawHere.y + drawHere.height)) {
			...
			if (line == lineWithAnno) {
				...
				if (anno != null) {
					// draw the glyph only when the annotation type has its own
					// icon (no the default one)
					// or in case there is more than one annotations on the line
					if (!(count == 1 && anno.isDefaultGlyph())) {
						if (prepareImage(anno.getGlyph(), imgObserver))
							g.drawImage(anno.getGlyph(), xPos, y + (lineHeight - anno.getGlyph().getHeight(null)) / 2 + 1, null);
					}
				}

				// draw cycling button if there is more than one annotations on
				// the line
				if (count > 1)
					if (prepareImage(gutterButton, imgObserver) && prepareImage(anno.getGlyph(), imgObserver))
	...
	protected void checkSize() {
		...
		if (getWidthDimension() > dim.width || getHeightDimension() > dim.height) {
			resize();
		}
		...
		private void updateSelection(boolean newSelection) {
			...
			try {
				if (newSelection) {
					...
					if (currentEndLine == dragEndLine)
						return;
					// select backwards
					if (dragEndLine < dragStartLine) {
						if (selectForward) {
							// selection start should be at start of (dragLine +
							// 1)
					...
					// select forwards
						...
						;
						// for last line or more -1 is returned, so set to
						// docLength...
						if (offSet < 0) {
