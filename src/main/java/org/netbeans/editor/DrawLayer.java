...
/**
 * Draw layer applies changes to draw context during painting process. Each
 ...
 * context in <CODE>updateContext()</CODE> method. Draw layers form
...
public interface DrawLayer {
	...
	/**
	 * Get the name of the layer. The layers that should work together over one
	 * component must have the different names.
	 */
	public String getName();

	/**
	 * Whether the layer wants to use the last context's background till the end
	 * of the window or not.
	 */
	public boolean extendsEOL();

	/**
	 * Whether the layer marks the empty line with the background by half of the
	 * character.
	 */
	public boolean extendsEmptyLine();

	/**
	 * Get the next position at which the activity of the layer will change. It
	 * can return <tt>Integer.MAX_VALUE</tt> to mark that the activity will
	 * never change or if it will change only by draw-marks. When this position
	 * will be reached the <tt>isActive</tt> will be called.
	 */
	public int getNextActivityChangeOffset(DrawContext ctx);

	/**
	 * Called each time the paint begins for all layers in the layer chain
	 ...
	 * because <tt>isActive()</tt> will be called at the begining of the
	 * drawing when this method finishes.
	 */
	public void init(DrawContext ctx);

	/**
	 * Return whether the layer is active or not. This method is called at the
	 ...
	 * the valid <tt>mark</tt> parameter.
	 * 
	 * @param ctx
	 *            current context with the information about the drawing
	 * @param mark
	 *            draw-mark at the fragment-offset or null if called because of
	 *            the next-activity-change-offset.
	 */
	 ...
	 * the line-number <tt>lineOffset + 1</tt> but some layers can change it.
	 ...
	 * colors for the line-number. The context's <tt>getFragmentOffset()</tt>
	 ...
	 * 
	 * @param lineNumber
	 *            the number that will be drawn before the line's text. The
	 *            layer can change it by returning a different value.
	 * @param ctx
	 *            the draw context
	 */
	...
	public static abstract class AbstractLayer implements DrawLayer {
		...
		/**
		 * Name of this layer. The name of the layer must be unique among layers
		 * installed into EditorUI
		 */
		private String name;

		/**
		 * Next position where the layer should be notified to update its state.
		 */
