...
public class FindSupport {
	...
	/**
	 * Current finder creator. It can be changed by setFinderCreator.
	 */
	FinderCreator finderCreator;

	/**
	 * Were the find properties already initialized from settings?
	 */
	...
	public Map getDefaultFindProperties() {
		...
		props.put(SettingsNames.FIND_WHAT, Settings.getValue(kitClass, SettingsNames.FIND_WHAT));
		props.put(SettingsNames.FIND_REPLACE_WITH, Settings.getValue(kitClass, SettingsNames.FIND_REPLACE_WITH));
		props.put(SettingsNames.FIND_HIGHLIGHT_SEARCH, Settings.getValue(kitClass, SettingsNames.FIND_HIGHLIGHT_SEARCH));
		props.put(SettingsNames.FIND_INC_SEARCH, Settings.getValue(kitClass, SettingsNames.FIND_INC_SEARCH));
		props.put(SettingsNames.FIND_BACKWARD_SEARCH, Settings.getValue(kitClass, SettingsNames.FIND_BACKWARD_SEARCH));
		props.put(SettingsNames.FIND_WRAP_SEARCH, Settings.getValue(kitClass, SettingsNames.FIND_WRAP_SEARCH));
		props.put(SettingsNames.FIND_MATCH_CASE, Settings.getValue(kitClass, SettingsNames.FIND_MATCH_CASE));
		props.put(SettingsNames.FIND_WHOLE_WORDS, Settings.getValue(kitClass, SettingsNames.FIND_WHOLE_WORDS));
		props.put(SettingsNames.FIND_REG_EXP, Settings.getValue(kitClass, SettingsNames.FIND_REG_EXP));
		props.put(SettingsNames.FIND_HISTORY, Settings.getValue(kitClass, SettingsNames.FIND_HISTORY));
	...
	public FinderFactory.StringFinder getStringFinder(BaseDocument doc, Map props) {
		FinderFactory.StringFinder stringFinder = (FinderFactory.StringFinder) doc.getProperty(BaseDocument.STRING_FINDER_PROP);

		if (stringFinder == null) {
			stringFinder = getFinderCreator().createFinder(doc, getValidFindProperties(props));
	...
	public FinderFactory.StringFinder getStringBwdFinder(BaseDocument doc, Map props) {
		FinderFactory.StringFinder stringBwdFinder = (FinderFactory.StringFinder) doc.getProperty(BaseDocument.STRING_BWD_FINDER_PROP);

		if (stringBwdFinder == null) {
			stringBwdFinder = getFinderCreator().createBwdFinder(doc, getValidFindProperties(props));
	...
	public FinderFactory.BlocksFinder getBlocksFinder(BaseDocument doc, Map props) {
		FinderFactory.BlocksFinder blocksFinder = (FinderFactory.BlocksFinder) doc.getProperty(BaseDocument.BLOCKS_FINDER_PROP);

		if (blocksFinder == null) {
			blocksFinder = getFinderCreator().createBlocksFinder(doc, getValidFindProperties(props));
		}
		return blocksFinder;
	}
	...
	/**
	 * Get find property without performing initialization of find properties.
	...
	/**
	 * Add/replace properties from some other map to current find properties. If
		...
		if (b != null && b.booleanValue()) { // inc search enabled
			...
			if (c != null) {
				...
				try {
					...
					if (back) {
						pos = doc.find(finder, dot, 0); // !!! handle wrap
														// search
					}
					else {
						pos = doc.find(finder, dot, -1); // !!! handle wrap
															// search
					}
				...
				if (incLayer == null) {
					...
					if (!editorUI.addLayer(incLayer, DrawLayerFactory.INC_SEARCH_LAYER_VISIBILITY)) {
	 ...
	 *            search properties
	 * @param oppositeDir
	 *            whether search in opposite direction
	 */
	public boolean find(Map props, boolean oppositeDir) {
		...
		if (c != null) {
			...
			try {
				...
				if (blk != null) {
					if (caret instanceof BaseCaret) { // support extended
														// scroll mode
	 ...
	 * 
	 * @param startPos
	 *            position from which to search. It must be inside the block.
	 * @param blockStartPos
	 *            starting position of the block. It must be valid position
	 *            greater or equal than zero. It must be lower than or equal to
	 *            blockEndPos (except blockEndPos=-1).
	 * @param blockEndPos
	 *            ending position of the block. It can be -1 for the end of
	 *            document. It must be greater or equal than blockStartPos
	 *            (except blockEndPos=-1).
	 * @param props
	 *            search properties
	 * @param oppositeDir
	 *            whether search in opposite direction
	 ...
	 * @return either null when nothing was found or integer array with three
	 *         members ret[0] - starting position of the found string ret[1] -
	 *         ending position of the found string ret[2] - 1 or 0 when wrap was
	 *         or wasn't performed in order to find the string
	 */
	public int[] findInBlock(JTextComponent c, int startPos, int blockStartPos, int blockEndPos, Map props, boolean oppositeDir) throws BadLocationException {
		if (c != null) {
			props = getValidFindProperties(props);
			BaseDocument doc = (BaseDocument) c.getDocument();
			FinderFactory.StringFinder sf = oppositeDir ? getStringBwdFinder(doc, props) : getStringFinder(doc, props);
			int pos = -1;
	...
	public boolean replace(Map props, boolean oppositeDir) throws BadLocationException {
		...
		if (c != null) {
			...
			if (back) {
				// Handle repetitive search ('a' replaced by 'aa') by moving the
				// caret
		...
		if (c != null) {
			...
			try {
				...
				while (true) {
					...
					totalCnt++;
					int len = blk[1] - blk[0];
					boolean skip = false; // cannot remove (because of guarded
											// block)?
					try {
	...
	public static class DefaultFinderCreator implements FinderCreator {
		...
		/**
		 * Creates finder that makes position pairs of begining and end of the
		 * matched word.
		 */
		public FinderFactory.BlocksFinder createBlocksFinder(BaseDocument doc, Map searchProps) {
