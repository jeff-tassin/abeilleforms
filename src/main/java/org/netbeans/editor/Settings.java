...
/**
 * Configurable settings that editor uses. All the methods are static The editor
 ...
 * <tt>org.netbeans.editor.BaseKit</tt> kit, the begining of the whole kit
 ...
 * <tt>Settings</tt>. Any class inheritance hierarchy could be used here
 ...
 * defined as public String constants in <tt>SettingsNames</tt> class. The
...
public class Settings {
	...
	/**
	 * Core level used by the settings initializers. This is the level used for
	...
	/**
	 * Option level used by the settings initializers. This is the third level.
	...
	/**
	 * Internal map instance signing that initializer returned null map for
	 * particular kit. To sign this fact and not query initializer again, this
	 * simple map is used.
	 */
	...
	/**
	 * Add initializer instance to the list of current initializers. You can
	 ...
	 * 
	 * @param i
	 *            initializer to add to the current list of initializers
	 * @param level
	 *            initializer level. It defines in which order the initializers
	 *            will be called. There are currently three levels
	 *            <tt>CORE_LEVEL</tt>, <tt>SYSTEM_LEVEL</tt> and
	 *            <tt>USER_LEVEL</tt>. It's guaranteed that initializers with
	 *            the particular level will be called in the order shown above.
	 *            The order of the initializers at the same level is given by
	...
	/**
	 * Remove the initializer of the given name from all the levels where it
	 * occurs.
	 * 
	 * @param name
	 *            name of the initializer sorter to remove.
	 */
	...
	/**
	 * Add filter instance to the list of current filters. If there are already
	 ...
	 * 
	 * @param f
	 *            filter to add to the list of the filters
	 */
	 ...
	 * 
	 * @param kitClass
	 *            editor kit class for which the value of setting should be
	 *            retrieved. The null can be used as the root of the whole
	 *            hierarchy. (which is top of kit class hierarchy) before the
	 *            processing begins.
	 * @param settingName
	 *            name of the setting for which the value should be retrieved
	 ...
	 * @param settingName
	 *            name of the setting for which the value should be retrieved
	 * @param evaluateEvaluators
	 *            whether the evaluators should be evaluated or not
	 * @return the array containing KitAndValue instances describing the
	 *         particular setting's value on the specific kit level.
	 */
	...
	/**
	 * Set the new value for property on kit level. The old and new values are
	 * compared and if they are equal the setting is not changed and nothing is
	 * fired.
	 * 
	 * @param kitClass
	 *            editor kit class for which the value of setting should be set.
	 *            The null can be used as the root of the whole hierarchy.
	 * @param settingName
	 *            the string used for searching the value
	 * @param newValue
	 *            new value to set for the property; the value can be null to
	 *            clear the value for the specified kit
	 */
	...
	/**
	 * Don't change the value of the setting, but fire change event. This is
	...
	public static synchronized void touchValue(Class kitClass, String settingName) {
		fireSettingsChange(kitClass, settingName, null, null); // kit class
																// currently not
																// used
	}

	/**
	 * Set the value for the current kit and propagate it to all the children of
	 ...
	 * @param kitClass
	 *            editor kit class for which the value of setting should be set.
	 *            When given null, it's automatically set to BaseKit.class
	 *            (which is top of kit class hierarchy) before the processing
	 *            begins. Therefore you get exactly the same results for
	 *            kitClass == null and kitClass == BaseKit.class.
	 * @param settingName
	 *            the string used for searching the value
	 * @param newValue
	 *            new value to set for the property; the value can be null to
	...
	/**
	 * Run the given runnable. All the changes in the settings are not fired
	 * until the whole runnable completes. Nesting of <tt>update()</tt> call
	...
	/**
	 * Reset all the settings and fire the change of the settings so that all
	...
	/**
	 * Initializer of the settings updates the map filled with settings for the
	 * particular kit class when asked. If the settings are being initialized
	 * all the initializers registered by the <tt>Settings.addInitializer()</tt>
	 * are being asked to update the settings-map through calling their
	 * <tt>updateSettingsMap()</tt>.
	 */
	public static interface Initializer {

		/**
		 * Each initializer must have a name. The name should be unique. The
		 * name is used for identifying the initializer during removal and sort
		 * operations and for debuging purposes.
		 */
		 ...
		 * @param settingsMap
		 *            map holding [setting-name, setting-value] pairs. The map
		 *            can be empty if this is the first initializer that updates
		 *            it or if no previous initializers updated it.
		 */
	...
	/**
	 * Sort the settings initializers that were added to the settings. There can
	 * be only one sorter for the Settings, but it can delegate to previously
	 * registered sorter.
	 */
	...
	/**
	 * Evaluator can be used in cases when value of some setting depends on the
	 * value for other setting and it allows to compute the value dynamically
	 * based on the other setting(s) value. The <tt>Evaluator</tt> instance
	 * can be used as the value in the <tt>Settings.setValue()</tt> call. In
	 * that case the call to the <tt>Settings.getValue()</tt> call will
	 * 'evaluate' the Evaluator by calling its <tt>getValue()</tt>.
	 */
	public static interface Evaluator {
		 ...
		 * 
		 * @param kitClass
		 *            kit class for which the setting is being retrieved.
		 * @param settingName
		 *            name of the setting to retrieve. Although the Evaluator
		 *            are usually constructed only for the concrete setting,
		 *            this parameter allows creation of the Evaluator for
		 *            multiple settings.
		 * @return the value for the requested setting. The substitution is not
		 *         attempted again, so the return value cannot be another
		 *         Evaluator instance. If the returned value is null, the same
		 *         action is taken as if there would no value set on the
		 *         particular kit level.
		 * 
		 */
	...
	/**
	 * Filter is applied on every value or KitAndValue pairs returned from
	 * getValue(). The filter can be registered by calling
	 * <tt>Settings.addFilter()</tt>. Each call to
	 * <tt>Settings.getValue()</tt> will first retrieve the value and then
	 * call the <tt>Filter.filterValue()</tt> to get the final value. Each
	 * call to <tt>Settings.getValueHierarchy()</tt> will first retrieve the
	 * kit-and-value array and then call the
	 * <tt>Filter.filterValueHierarchy()</tt>. If more filters are registered
	 * they are all used in the order they were added.
	 */
	public static interface Filter {
		 ...
		 * 
		 * @param kitClass
		 *            class of the kit for which the value is retrieved
		 * @param settingName
		 *            name of the retrieved setting
		 * @param value
		 *            value to be optionally filtered
		 */
		public Object filterValue(Class kitClass, String settingName, Object value);

		/**
		 * Filter array of kit-class and value pairs. The pairs can be
		 * completely substituted with an array with different length and
		 * different members.
		 * 
		 * @param kitClass
		 *            class of the kit for which the value is retrieved
		 ...
		 *            name of the retrieved setting
		 * @param kavArray
		 *            kit-class and value array to be filtered
		 */
		public KitAndValue[] filterValueHierarchy(Class kitClass, String settingName, KitAndValue[] kavArray);
