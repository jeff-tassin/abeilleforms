...
public class Annotations implements DocumentListener {
	...
	public Annotations(BaseDocument doc) {
		...
		doc.addLayer(new DrawLayerFactory.AnnotationLayer(doc), DrawLayerFactory.ANNOTATION_LAYER_VISIBILITY);
	...
	public void addAnnotation(AnnotationDesc anno) {
		...
		// create mark for this annotation. One mark can be shared by more
		// annotations
		...
		// fine LineAnnotations instance corresponding to the line of this
		// annotation
		// or create new LineAnnotations if this is first annotation on this
		// line
	...
	/**
	 * Finds active annotation for the Mark. It is called from DrawLayer when it
	 * found the DrawMark
	 */
	...
	/**
	 * Returns the active annotation for the given line number. It is called
	 * from the glyph gutter
	 */
	...
	/**
	 * Move annotation in front of others. The activated annotation is moved in
	 * front of other annotations on the same line
	 */
	...
	/**
	 * Activate next annotation on the line. Used for cycling through the
	 * annotations
	 */
	...
	/**
	 * Checks the number of removed lines and recalculate LineAnnotations.line
	 * property
	 */
	...
	/**
	 * Checks the number of inserted lines and recalculate LineAnnotations.line
	 * property
	 */
	...
	/**
	 * Creates menu item for the given action. It must handle the BaseActions,
	 * which have localized name stored not in Action.NAME property.
	 */
	...
	public JPopupMenu createPopupMenu(BaseKit kit, int line) {
		return createMenu(kit, line).getPopupMenu();
	}
	...
	public JMenu createMenu(BaseKit kit, int line) {
		...
		JMenu pm = new JMenu(LocaleSupport.getString("generate-gutter-popup"));
		Action[] actions;
	...
	/**
	 * Manager of all annotations attached to one line. Class stores the
	 * references to all annotations from one line in List and also stores which
	 * annotation is active, count of visible annotations and line number.
	 */
	static public class LineAnnotations extends Object {
		...
		/**
		 * Remove annotation from this line. Refresh the active one and count of
		 * visible.
		 */
		public void removeAnnotation(AnnotationDesc anno) {
			if (anno == active)
		...
		/**
		 * Searches all combination annotation type and sort them by
		 * getCombinationOrder into combTypes array which is passed as paramter.
		 */
		private void fillInCombinationsAndOrderThem(LinkedList combTypes) {
			...
			for (Iterator it = AnnotationTypes.getTypes().getAnnotationTypeNames(); it.hasNext();) {
				...
				if (combs != null && type.isWholeLine() && (combs.length >= 2 || (combs.length == 1 && combs[0].isAbsorbAll()))) {
		...
		/**
		 * For the given combination annotation type and list of annotations it
		 * finds all annotations which are combined by this combination and
		 * inserts into list of annotations new combined annotation which wraps
		 * combined annotations. The result list of annotations can contain null
		 * values for annotations which were combined.
		 */
		private boolean combineType(AnnotationType combType, LinkedList annosDupl) {
			...
			// check that there is match between line annos & all types
			...
			for (i = 0; i < combs.length; i++) {
				...
				// check that for one specified combination type there exist
				// some annotation
				for (j = 0; j < annosDupl.size(); j++) {
					...
					// check whether this annotation matches the specified
					// combination type
					if (comb.getName().equals(anno.getAnnotationType())) {
						countOfAnnos++;

						// now check if the combination has specified some
						// minimum count of annos
			...
			if (matchedComb) {
				...
				for (i = 0; i < combinedAnnos.size(); i++) {
					...
					if (annoComb == null) {
						...
						annosDupl.set(annosDupl.indexOf(combinedAnnos.get(i)), annoComb); // replace
																							// the
																							// original
																							// annotation
																							// by
						...
						annosDupl.set(annosDupl.indexOf(combinedAnnos.get(i)), null); // remove
																						// annotations
																						// which
																						// were
																						// combined
		...
		/**
		 * Refresh the active annotation and count of visible annotations. This
		 * method is used after change of annotation type of some annotation on
		 * this line
		 */
		public void refreshAnnotations() {
			...
			if (!AnnotationTypes.getTypes().isCombineGlyphs().booleanValue()) {
				...
				// first, fill in the array with combination types sorted by the
				// order
				...
				// add remaining not combined annotations into the line
				// annotations array
		...
		/**
		 * Is this given mark still referenced by some annotation or it can be
		 * removed from the draw mark chain
		 */
	...
	public interface AnnotationsListener extends EventListener {
		 ...
		 * are only informed that something has changed and that the change must
		 * be reflected somehow (most probably by complete redraw).
		 */
	...
	/**
	 * Annotation which is used for representation of combined annotations. Some
	 * basic operations like getLine etc. are delegated to one of the
