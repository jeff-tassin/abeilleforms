...
/**
 * Actions that are not considered basic and therefore they are not included
 * directly in BaseKit, but here. Their names however are still part of BaseKit.
 * 
...
public class ActionFactory {
				...
				if (caret.isSelectionVisible()) { // block selected
					...
					try {
						...
						if (firstNW == -1 || (firstNW >= caret.getDot()))
							...
							// TODO:
							// after we will have action which will do opposite
							// to "tab" action
							// means it check whether before the caret is whole
							// tab which can
	...
	public static class RemoveWordAction extends BaseAction {
		...
		public RemoveWordAction() {
			super(BaseKit.removeWordAction, MAGIC_POSITION_RESET | ABBREV_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	public static class RemoveLineBeginAction extends BaseAction {
		...
		public RemoveLineBeginAction() {
			super(BaseKit.removeLineBeginAction, MAGIC_POSITION_RESET | ABBREV_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
			...
			if (target != null) {
				...
				try {
					...
					if (dotPos == bolPos) { // at begining of the line
						if (dotPos > 0) {
							doc.remove(dotPos - 1, 1); // remove previous
														// new-line
						}
					}
						...
						if (Analyzer.isWhitespace(chars, 0, chars.length)) {
							doc.remove(bolPos, dotPos - bolPos); // remove
																	// whitespace
						}
						else {
	...
	public static class RemoveLineAction extends BaseAction {
		...
		public RemoveLineAction() {
			super(BaseKit.removeLineAction, MAGIC_POSITION_RESET | ABBREV_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	public static class RemoveSelectionAction extends BaseAction {
		...
		public RemoveSelectionAction() {
			super(BaseKit.removeSelectionAction, MAGIC_POSITION_RESET | ABBREV_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
	...
	public static class GotoNextBookmarkAction extends BaseAction {
		...
		public GotoNextBookmarkAction(String nm, boolean select) {
			super(BaseKit.gotoNextBookmarkAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
			this.select = select;
		}
	...
	public static class RunMacroAction extends BaseAction {
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			if (!runningActions.add(macroName)) { // this macro is already
													// running, beware of loops
				error(target, "loop");
				return;
			}
	...
	public static class AbbrevExpandAction extends BaseAction {
		...
		public AbbrevExpandAction() {
			super(BaseKit.abbrevExpandAction, MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	public static class ChangeCaseAction extends BaseAction {
		...
		public ChangeCaseAction(String name, int changeCaseMode) {
			super(name, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
	...
	public static class FindNextAction extends BaseAction {
		...
		public FindNextAction() {
			super(BaseKit.findNextAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	public static class FindPreviousAction extends BaseAction {
		...
		public FindPreviousAction() {
			super(BaseKit.findPreviousAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	/**
	 * Finds either selection or if there's no selection it finds the word where
	...
	public static class ToggleHighlightSearchAction extends BaseAction {
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			if (target != null) {
				Boolean cur = (Boolean) FindSupport.getFindSupport().getFindProperty(SettingsNames.FIND_HIGHLIGHT_SEARCH);
				...
				FindSupport.getFindSupport().putFindProperty(SettingsNames.FIND_HIGHLIGHT_SEARCH, cur);
			}
		}
	}
	...
	public static class UndoAction extends BaseAction {
		...
		public UndoAction() {
			super(BaseKit.undoAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	public static class RedoAction extends BaseAction {
		...
		public RedoAction() {
			super(BaseKit.redoAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
	...
	public static class FormatAction extends BaseAction {
		...
		public FormatAction() {
			super(BaseKit.formatAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET);
			...
			if (target != null) {
				...
				try {
					...
					while (pos < endPosition.getOffset()) {
						...
						if (gdoc != null) { // adjust to start of the next
											// guarded block
	...
	public static class SelectNextParameterAction extends BaseAction {
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			if (target != null) {
				...
				try {
					if (dotPos > 0) {
						if (doc.getChars(dotPos - 1, 1)[0] == ',') { // right
																		// after
																		// the
																		// comma
					...
					if (selectStartPos >= 0) {
						int selectEndPos = doc.find(new FinderFactory.CharArrayFwdFinder(new char[] { ',', ')' }), selectStartPos, -1);
						if (selectEndPos >= 0) {
	...
	public static class InsertDateTimeAction extends BaseAction {
		...
		public InsertDateTimeAction() {
			super(BaseKit.insertDateTimeAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET);
		}
