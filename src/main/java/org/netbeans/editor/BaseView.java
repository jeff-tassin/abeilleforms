...
public abstract class BaseView extends View {
	...
	/**
	 * Index of this view as child view in its parent. This gives the parent
	...
	public abstract void modelToViewDG(int pos, DrawGraphics dg) throws BadLocationException;
	...
	/**
	 * Returns binary composition of regions that should be painted. It can be
	 * binary composition of INSETS_TOP, MAIN_AREA and INSETS_BOTTOM.
	 * 
	 * @param g
	 *            Graphics to paint through
	 * @param clip
	 *            clipping area of graphics object
	 * 
	 */
	 ...
	 *            clipping area of graphics object
	 * @param paintAreas
	 *            binary composition of paint areas
	 */
	protected abstract void paintAreas(Graphics g, int clipY, int clipHeight, int paintAreas);

	/**
	 * It divides painting into three areas: INSETS_TOP, MAIN_AREA,
	 * INSETS_BOTTOM. It paints them sequentially using <CODE>paintArea()</CODE>
	 * method until it returns false. This implementation also supposes that
	 * <CODE>allocation</CODE> is instance of <CODE>Rectangle</CODE> to save
	...
	/**
	 * This function is used to correct information about which index this child
	...
	/**
	 * Get child view's y base value. This function is called by children of
	 * this view to get its y offset.
	 * 
	 * @param view
	 *            is child view of this view for which the offset should be
	 *            computed.
	 * @param helperInd
	 *            is index that child view has cached to ease the parent view to
	 *            search for it in its children array. If this index is correct,
	 *            parent uses it. If it's incorrect parent view searches through
	 *            the whole array of its children to find the child. It then
	 *            calls children's <CODE>setParentInd()</CODE> to correct its
	 *            location index.
	 */
	protected abstract int getViewStartY(BaseView view, int helperInd);

	/**
	 * Informs the view that if it had cached start y of itself it should
	 * invalidate it as it is no longer valid and it should call <CODE>getViewStartY()</CODE>
	...
	/**
	 * Update height of main area as result of some important change. Propagate
	 ...
	 * 
	 * @param origView
	 *            view for which the displayHierarchy method was originally
	 *            called. It is marked by asterisk.
	 * @param col
	 *            column offset
	 * @param index
	 *            index of this view in parent children[] array
	 */
