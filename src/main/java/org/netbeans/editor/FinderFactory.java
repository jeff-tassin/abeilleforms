...
public class FinderFactory {
	...
	/**
	 * Abstract finder implementation. The only <CODE>find()</CODE> method
	 * must be redefined.
	 */
	...
	/**
	 * Finder for going n-lines forward where n is greater or equal 1 and
	 * finding and storing the BOL position of the line and then going till EOL
	 * is found.
	 */
	...
	/**
	 * Finder for going back to the first new line which helps to find the
	...
	/**
	 * Finder for getting visual column value for particular position. The
	 * starting position for find must be the start of particular line. The
	 * limit position should be set to position for which the visual column is
	 * requested. This method can be used only in case the font is superfixed
	 * i.e. all the characters of all font styles have the same width.
	 */
	...
	/**
	 * Finder for getting position from visual column knowledge. It is kind of
	 * reverse finder for <CODE>PosVisColFwdFinder</CODE>. The starting
	 * position for find should be the start of particular line. The found
	 * position will be that position in document that corresponds to the column
	 * position. This method can be used only in case the font is superfixed
	 * i.e. all the characters of all font styles have the same width.
	 */
	...
	public static abstract class GenericFwdFinder extends AbstractFinder {
		...
		/**
		 * This function decides if it found a desired string or not. The
		 * function receives currently searched character and flag if it's the
		 * last one that is searched or not.
		 * 
		 * @return if the function decides that it found a desired string it
		 *         sets <CODE>found = true</CODE> and returns how many
		 *         characters back the searched string begins in forward
		 *         direction (0 stands for current character). For example if
		 *         the function looks for word 'yes' and it gets 's' as
		 *         parameter it sets found = true and returns -2. If the string
		 *         is not yet found it returns how many characters it should go
	...
	public static abstract class GenericBwdFinder extends AbstractFinder {
		 ...
		 * @return if the function decides that it found a desired string it
		 *         sets <CODE>found = true</CODE> and returns how many
		 *         characters back the searched string begins in backward
		 *         direction (0 stands for current character). It is usually 0
		 *         as the finder usually decides after the last required
		 *         character but it's not always the case e.g. for
		 *         whole-words-only search it can be 1 or so. If the string is
		 *         not yet found it returns how many characters it should go in
		 *         backward direction (in this case it would usually be -1). The
		 *         next searched character will be that one requested.
		 */
		protected abstract int scan(char ch, boolean lastChar);
	...
	public static abstract class GenericFinder extends AbstractFinder {
		...
		/**
		 * The method that gets the actual character and whether that character
		 * is the last in the search. It can generally set the found flag to
		 * true to signal the successive search or it can return positive number
		 * to go forward or negative number to go back.
		 */
		protected abstract int scan(char ch, boolean lastChar);
	}
	...
	/**
	 * Find start of the word. This finder can be used to go to previous word or
		...
		protected int scan(char ch, boolean lastChar) {
			...
			if (ch == chars[stringInd]) {
				...
				if (stringInd == chars.length) { // found whole string
					found = true;
					return 1 - stringInd; // how many chars back the string
											// starts
				}
	...
	/**
	 * String forward finder that finds whole words only. There are some speed
			...
			if (stringInd == 0) { // special case for first char
				...
				if (ch == chars[stringInd]) { // matches current char
					...
					if (stringInd == chars.length) { // found whole string
						if (lastChar) {
							found = true;
							return 1 - chars.length; // how many chars back
														// the string starts
						}
						else {
							wordFound = true;
					...
					stringInd = 0;
					insideWord = firstCharWordPart;
					return back; // go back to search from the next to first
									// char
				}
			}
		}
	...
	/**
	 * String backward finder that finds whole words only. There are some speed
	 * optimizations attemted.
	 */
	public static final class WholeWordsBwdFinder extends GenericBwdFinder implements StringFinder {
		...
		protected int scan(char ch, boolean lastChar) {
			...
			if (stringInd == endInd) { // special case for last char
				if (ch != chars[endInd] || insideWord) { // first char
															// doesn't match
					insideWord = doc.isIdentifierPart(ch);
					return -1;
				}
	...
	/**
	 * String forward finder that finds whole words only and that creates
	 * position blocks. There are some speed optimizations attempted.
	 */
	public static final class WholeWordsBlocksFinder extends AbstractBlocksFinder {
		...
		public int find(int bufferStartPos, char buffer[], int offset1, int offset2, int reqPos, int limitPos) {
			...
			while (offset >= offset1 && offset < offset2) {
				...
				if (stringInd == 0) { // special case for first char
					if (ch != chars[0] || insideWord) { // first char doesn't
														// match
						insideWord = doc.isIdentifierPart(ch);
						offset++;
					}
					...
					if (ch == chars[stringInd]) { // matches current char
						stringInd++;
						if (stringInd == chars.length) { // found whole
															// string
							if (offset == limitOffset) {
	...
	/**
	 * Finder that looks for some search expression expressed by string. It can
	 * be either simple string or some form of regular expression expressed by
	 * string.
	 */
	public interface StringFinder extends Finder {

		/**
		 * Get the length of the found string. This is useful for regular
		 * expressions, because the length of the regular expression can be
		 * different than the length of the string that matched the expression.
		 */
		public int getFoundLength();
	...
	/**
	 * Finder that constructs [begin-pos, end-pos] blocks. This is useful for
	 * highlight-search draw layer. The block-finders are always forward-search
	 * finders.
	 */
	public interface BlocksFinder extends Finder {

		/**
		 * Set the array into which the finder puts the position blocks. If the
		 * length of array is not sufficient the finder extends the array. The
		 * last block is set to [-1, -1].
		 */
		public void setBlocks(int[] blocks);

		/**
		 * Get the array filled with position blocks. It is either original
		 * array passed to setBlocks() or the new array if the finder extended
		 * the array.
		 */
		public int[] getBlocks();
