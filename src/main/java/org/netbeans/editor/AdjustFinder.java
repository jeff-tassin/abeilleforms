 ...
 * The finder can be used in the <tt>BaseDocument.find()</tt> which calls its
 ...
 * If the search is void i.e. <tt>doc.find(finder, pos, pos)</tt> is called,
 * no adjust-methods are called, only the <tt>reset()</tt> is called. For
 * backward search the start-position is higher than the limit-position. The
 * relation <tt>startPos &lt; endPos</tt> defines whether the search will be
...
public interface AdjustFinder extends Finder {
	...
	/**
	 * Adjust start position of the search to be either the same or lower. This
	 ...
	 * @param startPos
	 *            start position originally requested in
	 *            <tt>BaseDocument.find()</tt>.
	 * @return possibly modified start position. The returned position must be
	 *         the same or lower than the original start position for forward
	 *         search and the same or high.
	 */
	public int adjustStartPos(BaseDocument doc, int startPos);

	/**
	 * Adjust the limit position of the search (it's the position where the
	 * search will end) to be either the same or greater.
	 * 
	 * @param doc
	 *            document to search on
	 * @param limitPos
	 *            limit position originally requested in
	 *            <tt>BaseDocument.find()</tt>
