...
public class MarkFactory {
	...
	/**
	 * Syntax mark holds info about scan state of syntax scanner. This helps in
	 * redraws because reparsing after insert/delete is done only from nearest
	 * left syntax mark. Moreover rescaning is done only until there are marks
	 * with different scan state. As soon as mark is found with same parsing
	 * info as rescanning scanner has, parsing ends.
	 */
	...
	/**
	 * Activation mark for particular layer. When layer is not active its
	 * updateContext() method is not called.
	 */
	public static class DrawMark extends ContextMark {
		...
		/**
		 * Reference to extended UI if this draw mark is info-specific or null
		 * if it's document-wide.
		 */
	...
	static class LineMark extends Mark {
		...
		BaseDocument.LineElement lineElemRef; // non-weak ref due to #17351
