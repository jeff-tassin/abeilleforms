...
/**
 * Class that can hold the list of event listeners in a "weak" manner. The
 * advantage is that the listener doesn't need to be explicitly removed. Because
 * of the use of the <tt>WeakReference</tt> it gets forgotten automatically if
 * it was garbage collected. There must be at least one non-weak reference to
 * the listener (otherwise it would become garbage-collected). One of the ways
 * is to store the listener in some instance variable in the class that adds the
 * listener. Please note that the methods <tt>getListenerCount()</tt> and
 * <tt>getListenerCount(Class t)</tt> give the count that doesn't reflect
 * whether some listeners were garbage collected.
 * 
...
public class WeakEventListenerList extends EventListenerList {
	...
	public synchronized Object[] getListenerList() {
		...
		for (int i = 1; i < listenerSize; i += 2) {
			...
			if (l != null) {
				...
				/*
				 * Remove the listener and its class. This could be done in a
				 * more efficient but much less readable way batching the
				 * successive removes into one.
				 */
				System.arraycopy(listenerList, i + 1, listenerList, i - 1, listenerSize - i - 1);
				listenerSize -= 2;
				i -= 2;
			}
		}
		...
		for (int i = 0; i < listenerSize; i++) {
			if (listenerList[i++] == t) {
				...
				if (l != null) {
					...
					/*
					 * Remove the listener and its class. This could be done in
					 * a more efficient but much less readable way batching the
					 * successive removes into one.
					 */
					System.arraycopy(listenerList, i + 1, listenerList, i - 1, listenerSize - i - 1);
					listenerSize -= 2;
	...
	public synchronized void remove(Class t, EventListener l) {
		...
		for (int i = listenerSize - 2; i >= 0; i -= 2) {
			if ((listenerList[i] == t) && (((WeakReference) listenerList[i + 1]).get()).equals(l)) {
				index = i;
				break;
			}
		}
		...
		// If so, remove it
		if (index >= 0) {
			System.arraycopy(listenerList, index + 2, listenerList, index, listenerSize - index - 2);
			listenerSize -= 2;
		}
	}
