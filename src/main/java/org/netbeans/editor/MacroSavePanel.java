...
/**
 * The component for displaying and editing just recorded macro. It allows you
...
public class MacroSavePanel extends javax.swing.JPanel {
	...
	/**
	 * @param sequences
	 *            List of KeyStroke[] bounds to this macro
	 */
	...
	class KeySequenceRequester {
		...
		KeySequenceRequester() {
			...
			dial = DialogSupport.createDialog(LocaleSupport.getString("MSP_AddTitle"), // NOI18N
					panel, true, buttons, false, -1, 2, new ActionListener() {
						public void actionPerformed(ActionEvent evt) {
							if (evt.getSource() == buttons[1]) { // Clear
																	// pressed
								panel.clear(); // Clear entered KeyStrokes,
												// start again
								panel.requestFocus(); // Make user imediately
														// able to enter new
														// strokes
							}
							else if (evt.getSource() == buttons[0]) { // OK
																		// pressed
								retVal = panel.getKeySequence();
								dial.dispose(); // Done
							}
							else if (evt.getSource() == buttons[2]) { // OK
																		// pressed
								retVal = null;
								dial.dispose(); // Done
							}
