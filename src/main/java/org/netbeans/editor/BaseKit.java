...
public class BaseKit extends DefaultEditorKit {
	...
	/**
	 * Name of the action moving caret to the first column on the line and
	...
	static SettingsChangeListener settingsListener = new SettingsChangeListener() {
		public void settingsChange(SettingsChangeEvent evt) {
			...
			boolean clearActions = (settingName == null || SettingsNames.CUSTOM_ACTION_LIST.equals(settingName) || SettingsNames.MACRO_MAP.equals(settingName));
	 ...
	 * component.
	 * 
	 * @return class of the component that will be searched in the parents in
	 *         the hierarchy or null to do no search.
	 */
	...
	public void install(JEditorPane c) {
		...
		if (noInputMethods != null) {
			...
			enableIM = SettingsUtil.getBoolean(this.getClass(), SettingsNames.INPUT_METHODS_ENABLED, true);
		}
	...
	protected void executeInstallActions(JEditorPane c) {
		Settings.KitAndValue[] kv = Settings.getValueHierarchy(this.getClass(), SettingsNames.KIT_INSTALL_ACTION_NAME_LIST);
		for (int i = kv.length - 1; i >= 0; i--) {
			List actList = (List) kv[i].value;
			actList = translateActionNameList(actList); // translate names to
														// actions
			if (actList != null) {
	...
	protected void executeDeinstallActions(JEditorPane c) {
		Settings.KitAndValue[] kv = Settings.getValueHierarchy(this.getClass(), SettingsNames.KIT_DEINSTALL_ACTION_NAME_LIST);
		for (int i = kv.length - 1; i >= 0; i--) {
			List actList = (List) kv[i].value;
			actList = translateActionNameList(actList); // translate names to
														// actions
	...
	/**
	 * Create actions that this kit supports. To use the actions of the parent
	...
	protected Action[] createActions() {
		return new Action[] { new DefaultKeyTypedAction(), new InsertContentAction(), new InsertBreakAction(), new InsertTabAction(),
				new DeleteCharAction(deletePrevCharAction, false), new DeleteCharAction(deleteNextCharAction, true), new ReadOnlyAction(),
				new WritableAction(), new CutAction(), new CopyAction(), new PasteAction(), new BeepAction(), new UpAction(upAction, false),
				new UpAction(selectionUpAction, true), new PageUpAction(pageUpAction, false), new PageUpAction(selectionPageUpAction, true),
				new DownAction(downAction, false), new DownAction(selectionDownAction, true), new PageDownAction(selectionPageDownAction, true),
				new PageDownAction(pageDownAction, false), new ForwardAction(forwardAction, false), new ForwardAction(selectionForwardAction, true),
				new BackwardAction(backwardAction, false), new BackwardAction(selectionBackwardAction, true),
				new BeginLineAction(lineFirstColumnAction, false, true), new BeginLineAction(selectionLineFirstColumnAction, true, true),
				new BeginLineAction(beginLineAction, false), new BeginLineAction(selectionBeginLineAction, true), new EndLineAction(endLineAction, false),
				new EndLineAction(selectionEndLineAction, true), new BeginAction(beginAction, false), new BeginAction(selectionBeginAction, true),
				new EndAction(endAction, false), new EndAction(selectionEndAction, true), new NextWordAction(nextWordAction, false),
				new NextWordAction(selectionNextWordAction, true), new PreviousWordAction(previousWordAction, false),
				new PreviousWordAction(selectionPreviousWordAction, true), new BeginWordAction(beginWordAction, false),
				new BeginWordAction(selectionBeginWordAction, true), new EndWordAction(endWordAction, false), new EndWordAction(selectionEndWordAction, true),
				new SelectWordAction(), new SelectLineAction(), new SelectAllAction(), new ActionFactory.RemoveTabAction(),
				new ActionFactory.RemoveWordAction(), new ActionFactory.RemoveLineBeginAction(), new ActionFactory.RemoveLineAction(),
				new ActionFactory.RemoveSelectionAction(), new ActionFactory.ToggleTypingModeAction(), new ActionFactory.ToggleBookmarkAction(),
				new ActionFactory.GotoNextBookmarkAction(gotoNextBookmarkAction, false), new ActionFactory.AbbrevExpandAction(),
				new ActionFactory.AbbrevResetAction(), new ActionFactory.ChangeCaseAction(toUpperCaseAction, Utilities.CASE_UPPER),
				new ActionFactory.ChangeCaseAction(toLowerCaseAction, Utilities.CASE_LOWER),
				new ActionFactory.ChangeCaseAction(switchCaseAction, Utilities.CASE_SWITCH), new ActionFactory.FindNextAction(),
				new ActionFactory.FindPreviousAction(), new ActionFactory.FindSelectionAction(), new ActionFactory.ToggleHighlightSearchAction(),
				new ActionFactory.UndoAction(), new ActionFactory.RedoAction(), new ActionFactory.WordMatchAction(wordMatchNextAction, true),
				new ActionFactory.WordMatchAction(wordMatchPrevAction, false), new ActionFactory.ShiftLineAction(shiftLineLeftAction, false),
				new ActionFactory.ShiftLineAction(shiftLineRightAction, true), new ActionFactory.AdjustWindowAction(adjustWindowTopAction, 0),
				new ActionFactory.AdjustWindowAction(adjustWindowCenterAction, 50), new ActionFactory.AdjustWindowAction(adjustWindowBottomAction, 100),
				new ActionFactory.AdjustCaretAction(adjustCaretTopAction, 0), new ActionFactory.AdjustCaretAction(adjustCaretCenterAction, 50),
				new ActionFactory.AdjustCaretAction(adjustCaretBottomAction, 100), new ActionFactory.FormatAction(),
				new ActionFactory.FirstNonWhiteAction(firstNonWhiteAction, false), new ActionFactory.FirstNonWhiteAction(selectionFirstNonWhiteAction, true),
				new ActionFactory.LastNonWhiteAction(lastNonWhiteAction, false), new ActionFactory.LastNonWhiteAction(selectionLastNonWhiteAction, true),
				new ActionFactory.SelectIdentifierAction(), new ActionFactory.SelectNextParameterAction(), new ActionFactory.JumpListPrevAction(),
				new ActionFactory.JumpListNextAction(), new ActionFactory.JumpListPrevComponentAction(), new ActionFactory.JumpListNextComponentAction(),
				new ActionFactory.ScrollUpAction(), new ActionFactory.ScrollDownAction(), new ActionFactory.StartMacroRecordingAction(),
				new ActionFactory.StopMacroRecordingAction(), new ActionFactory.InsertDateTimeAction(), new ActionFactory.GenerateGutterPopupAction(),
	...
	protected Action[] getCustomActions() {
		Settings.KitAndValue kv[] = Settings.getValueHierarchy(this.getClass(), SettingsNames.CUSTOM_ACTION_LIST);
		if (kv.length == 0) {
			return null;
		}
		if (kv.length == 1) {
	...
	/**
	 * Get actions associated with this kit. createActions() is called to get
	...
	/**
	 * Update the actions right after their creation was finished. The
	 * <code>getActions()</code> and <code>getActionByName()</code> can be
	 * used safely in this method. The implementation must call
	 * <code>super.updateActions()</code> so that the updating in parent is
	 * performed too.
	 */
	...
	public static class DefaultKeyTypedAction extends BaseAction {
				...
				if ((cmd != null) && (cmd.length() == 1)) {
					...
					if ((ch >= 0x20) && (ch != 0x7F)) { // valid character
						...
						try {
							...
							if (doInsert) {
								if (caret.isSelectionVisible()) { // valid
																	// selection
		...
		/**
		 * Check whether there was any important character typed so that the
		 * line should be possibly reformatted.
		 */
			...
			if (target != null) {
				...
				if (caret.isSelectionVisible()) { // block selected
					...
					try {
						...
						if (indent == -1) {
							if (upperCol > caretCol) { // upper indent is
														// greater
								indent = upperCol;
							}
			...
			if (target != null) {
				...
				try {
					...
					if (!select && caret.isSelectionVisible()) {
						...
						pos = caret.getDot();
					int dot = target.getUI().getNextVisualPositionFrom(target, pos, null, SwingConstants.WEST, null);
					if (select) {
						caret.moveDot(dot);
					}
					else {
	...
	public static class BeginLineAction extends BaseAction {
		...
		/**
		 * Whether the action should go to the begining of the text on the line
		 * or to the first column on the line
		 */
	...
	public static class BeginAction extends BaseAction {
		...
		public BeginAction(String nm, boolean select) {
			super(nm, MAGIC_POSITION_RESET | ABBREV_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET | SAVE_POSITION | CLEAR_STATUS_TEXT);
			this.select = select;
		}
	...
	public static class SelectLineAction extends KitCompoundAction {
		...
		public SelectLineAction() {
			super(selectLineAction, new String[] { lineFirstColumnAction, selectionEndLineAction, selectionForwardAction });
		}
