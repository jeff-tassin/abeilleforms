...
/**
 * Definition of the annotation type. Annotation type is defined by attributes
 * like highlight color, foreground color, glyph icon, etc. Each annotation
 * added to document has reference to the name of the annotation type which
 * defines how the annotation will be drawn.
 * 
...
public class AnnotationType {
	...
	/** Property name for Highligh (Color) */
	...
	/**
	 * Property name for Combinations (AnnotationType.CombinationMember[]). If
	 * some annotation type has set this property, it means that editor must
	 * check if line contains all types which are defined in this array. If it
	 * contains, then all this annotation types become hidden and this type is
	 * shown instead of them.
	 */
	...
	/**
	 * Property holding the object which represent the source of this annotation
	 * type. This property is used during the saving of the changes in
	 * annotation type.
	 */
	...
	/**
	 * Gets Image which represent the glyph. This method is called only from AWT
	 * thead and so it is not necessary to synchronize it.
	 */
	...
	/**
	 * Process change of some setting. It means that listeners are notified and
	 * change is saved.
	 */
	...
	public Coloring getColoring() {
		if (col == null)
			col = new Coloring(null, isInheritForegroundColor() ? null : getForegroundColor(), isUseHighlightColor() ? getHighlight() : null);
		return col;
	}
	 ...
	 * 
	 * @return minimum number of the optional annotation types which must be
	 *         matched
	 */
	 ...
	 * 
	 * @param ct
	 *            list of content type separeted by commas
	 */
	...
	/**
	 * Hepler class describing annotation type and whether all occurences of
	 * this type should be absorbed by combination or not. The annonation type
	 * which want to combine some other types, must define array of instances of
	 * this helper class. See AnnotationType.PROP_COMBINATIONS property.
	 */
