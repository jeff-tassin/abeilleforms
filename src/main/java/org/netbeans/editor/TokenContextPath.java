 ...
 * public constructor. The only entrypoint is through the <tt>get()</tt>
 ...
 * used instead of calling <tt>equals()</tt>.
 * 
...
public final class TokenContextPath {
	...
	/**
	 * Full name-prefix consisting of the prefixes of all the contexts in the
	 * path.
	 */
	...
	/**
	 * Retrieve the contexts of this context-path. The contexts of the
	 * context-array must NOT be modified in any way.
	 */
	...
	/**
	 * Get parent context-path that consists of all the contexts of this path
	 * except the last one.
	 */
	...
	/**
	 * Does this path contain the given path. It corresponds to the situation
	 * when the contexts of the given path are at the begining of this path.
	 */
	 ...
	 * 
	 * @param byPath
	 *            path that will replace the initial portion of this path. The
	 *            length of the portion is the same as the length of this
	 *            parameter.
	...
	public String getNamePrefix() {
		if (namePrefix == null) {
			if (contexts.length == 1) {
				...
				namePrefix = (contexts[contexts.length - 1].getNamePrefix() + getParent().getNamePrefix()).intern();
			}
		}
