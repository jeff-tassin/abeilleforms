...
public class MarkBlockChain {
	...
	/**
	 * Tests whether the position range is partly or fully inside some mark
	 * block from the chain.
	 * 
	 * @param startPos
	 *            starting position of tested area
	 * @param endPos
	 *            ending position of tested area for removal or same as startPos
	 *            when insert is made
		...
		while (true) {
			...
			if ((rel & MarkBlock.AFTER) != 0) { // after this mark block
				if (beforeNext) {
					...
					else { // continues with contBlk and this relation is pure
							// after
	...
	public void removeEmptyBlocks() {
		try {
			MarkBlock blk = chain;
			while (blk != null) {
				if (blk.startMark.getOffset() == blk.endMark.getOffset()) { // empty
																			// block
					blk = checkedRemove(blk); // remove current block and get
												// the next one
				}
				else {
					blk = blk.next;
	...
	protected MarkBlock createBlock(int startPos, int endPos) throws BadLocationException {
		return new MarkBlock(doc, createBlockStartMark(), createBlockEndMark(), startPos, endPos);
	}
	 ...
	 * 
	 * @param concat
	 *            whether concatenate adjacent blocks
	 */
	public void addBlock(int startPos, int endPos, boolean concat) {
		...
		try {
			...
			if ((rel & MarkBlock.BEFORE) != 0) { // before currentBlock or
													// continue_begin
			...
			else if ((rel & MarkBlock.AFTER) != 0) { // after currentBlock or
														// continue_end
				...
				if (currentBlock == null) { // no current block
					...
					while (blk != null) {
						if (blk != currentBlock) { // except self
							if (currentBlock.extend(blk, concat)) { // if they
																	// overlapped
								MarkBlock tempCurBlk = currentBlock;
								blk = checkedRemove(blk); // will clear
															// currentBlock
		...
		try {
			...
			while (((rel = compareBlock(startPos, endPos)) & MarkBlock.OVERLAP) != 0) {
				if ((rel & MarkBlock.THIS_EMPTY) != 0) { // currentBlock is
					...
					case MarkBlock.INNER: // tested block inside currentBlock
											// -> divide
	...
	public int adjustToBlockEnd(int pos) {
		...
		if (rel == MarkBlock.INSIDE_BEGIN || rel == MarkBlock.INNER) { // inside
																		// blk
