...
public class StatusBar implements PropertyChangeListener, SettingsChangeListener {
	...
	static final Border CELL_BORDER = BorderFactory.createCompoundBorder(BorderFactory.createBevelBorder(BevelBorder.LOWERED), BorderFactory.createEmptyBorder(
			0, 2, 0, 2));
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (kitClass != null) {
			...
			if (settingName == null || SettingsNames.STATUS_BAR_CARET_DELAY.equals(settingName)) {
				caretDelay = SettingsUtil.getInteger(kitClass, SettingsNames.STATUS_BAR_CARET_DELAY, SettingsDefaults.defaultStatusBarCaretDelay);
				if (caretL != null) {
			...
			if (settingName == null || SettingsNames.STATUS_BAR_VISIBLE.equals(settingName)) {
				boolean wantVisible = SettingsUtil.getBoolean(kitClass, SettingsNames.STATUS_BAR_VISIBLE, SettingsDefaults.defaultStatusBarVisible);
	...
	protected void initPanel() {
		...
		addCell(CELL_TYPING_MODE, new String[] { insText, ovrText }).setHorizontalAlignment(SwingConstants.CENTER);
	...
	/*
	 * Refresh the whole panel by removing all the components and adding only
	 * those that appear in the cell-list.
	 */
